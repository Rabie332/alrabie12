...
class AccountMove(models.Model):
    ...
    # l10n_sa_qr_code_str = fields.Char(string="Zatca QR Code", compute="_compute_qr_code_str")
    ...
    response_message_zatca = fields.Text(
        string="Response ZATCA", readonly=True, copy=False)
    cleared_invoice = fields.Text(
        string="Cleared XML", readonly=True, copy=False)
    l10n_sa_send_state = fields.Selection(
        [("reported", "Reported"), ("cleared", "Cleared"),
    ...
    warning_messages = fields.Text(
        string="Warning messages", readonly=True, copy=False)
    error_messages = fields.Text(
        string="Error messages", readonly=True, copy=False)
    ...
    position3 = fields.Boolean(
        string="3rd Party invoice", default=False, copy=False)
    position4 = fields.Boolean(
        string="Nominal invoice", default=False, copy=False)
    position5 = fields.Boolean(
        string="Exports invoice", default=False, copy=False)
    position6 = fields.Boolean(
        string="Summary invoice", default=False, copy=False)
    position7 = fields.Boolean(
        string="Self billed invoice", default=False, copy=False)
    ...
    def _check_InvoiceTypeCode(self):
        for record in self:
            if record.position5 and record.position7:
                raise UserError(
                    "Self-billing is not allowed for export invoices")
    ...
    def _get_remaining_hours(self):
        for record in self:
            if (
                ...
                limit_date = record.l10n_sa_confirmation_datetime + \
                    timedelta(hours=24)
                number_of_hours = (
                    limit_date - current_date).total_seconds() / 3600.0
    ...
    def get_signature(self, message, cert, PrivateKey):
        ...
        root = etree.fromstring(message.decode(
            "utf-8").encode("utf-8"), parser)
        ...
        key = load_pem_private_key(
            PrivateKey, password=None, backend=default_backend())
        digital_signature = key.sign(digest_msg, ec.ECDSA(SHA256()))
        encoded_digital_signature = self.ensure_str(
            base64.b64encode(digital_signature))
        # Step 3: Generate Certificate Hash
        cert_hash = hashlib.sha256(cert).hexdigest()
        encoded_cert_hash = self.ensure_str(
            base64.b64encode(cert_hash.encode()))
        # Step 4: Populate the Signed Properties Output
        certificate = load_pem_x509_certificate(
            BC + cert + EC, default_backend())
        ...
        SignedProperties_hash = hashlib.sha256(
            signed_properties_data_xml.encode()).hexdigest()
        EncodedSignedProperties_hash = self.ensure_str(
            base64.b64encode(SignedProperties_hash.encode()))
        ...
        try:
            ...
            tag3 = root.find("cbc:IssueDate", nss_cbc).text + \
                "T" + root.find("cbc:IssueTime", nss_cbc).text
            tag4 = root.xpath(
                "//cac:LegalMonetaryTotal/cbc:TaxInclusiveAmount", namespaces=nsmapo)[0].text
            tag5 = root.xpath("//cac:TaxTotal/cbc:TaxAmount",
                              namespaces=nsmapo)[0].text
            inv_uuid = root.find("cbc:UUID", nss_cbc).text
            invoiceType = root.xpath(
                "//cbc:InvoiceTypeCode/@name", namespaces=nsmapo)[0]
        ...
        QR = self.generate_zatca_code(
            isSimplified, tag1, tag2, tag3, tag4, tag5, tag6, tag7, tag8, tag9)
        qr_code = etree.Element(
            root_nsmap["cac"] + "AdditionalDocumentReference")
        qr_reference_id = etree.SubElement(qr_code, root_nsmap["cbc"] + "ID")
        qr_reference_id.text = "QR"
        qr_attach_node = etree.SubElement(
            qr_code, root_nsmap["cac"] + "Attachment")
        ...
        signature_method = etree.SubElement(
            signature, root_nsmap["cbc"] + "SignatureMethod")
        signature_method.text = "urn:oasis:names:specification:ubl:dsig:enveloped:xades"

        for subtag in root.findall("cac:AdditionalDocumentReference", nss_cac):
            if subtag.find("cbc:ID", nss_cbc).text == "PIH":
                qr_element = etree.tostring(
                    qr_code, pretty_print=True, encoding="UTF-8", xml_declaration=True)
                signature_element = etree.tostring(
                    signature, pretty_print=True, encoding="UTF-8", xml_declaration=True)
                root.insert(root.index(subtag) + 1,
                            etree.fromstring(qr_element))
                root.insert(root.index(subtag) + 2,
                            etree.fromstring(signature_element))
    ...
    def CallAPI(self, url, ClearanceStatus):
        ...
        try:
            response = requests.request(
                "POST", url, headers=headers, data=payload, auth=(user, pwd))
        except:
        ...
        if response.status_code in (200, 202):
            ...
            if response.status_code == 202:
                warning_messages = "\n".join(
                    [item["message"] for item in res["validationResults"]["warningMessages"]])
            ...
            error_messages = "\n".join(
                [item["message"] for item in res["validationResults"]["errorMessages"]])
    ...
    def CallClearanceAPI(self):
        ...
        if res:
            ...
            if self.cleared_invoice:
                ...
                invoice_hash_node = root.xpath(
                    '//*[@Id="invoiceSignedData"]/*[local-name()="DigestValue"]')[0]
                digest = invoice_hash_node.text
                self.l10n_sa_qr_code_str = cleared_qr
                self.sha256 = digest
                # To keep original xml content
                # self.xml_content = xml_cleared_invoice
                self.xml_file.datas = base64.b64encode(xml_cleared_invoice)
    ...
    def _get_count_invoice(self):
        invoice_sent = self.env["account.move"].search(
            [
                ("move_type", "in", ("out_invoice", "out_refund")),
                ("company_id", "=", self.company_id.id),
                ("l10n_sa_send_state", "!=", False),
        ...
        counter = max([edi_counter for edi_counter in invoice_sent.mapped(
            "edi_counter") if edi_counter > 0], default=0)
        return counter

    @api.model
    def _get_lines_invoice(self):
        count = len(self.invoice_line_ids.filtered(
    ...
    def _xml_add_header(self, parent_node, ns, ProfileID="reporting:1.0"):
        ...
        issue_date.text = self.l10n_sa_confirmation_datetime.strftime(
            "%Y-%m-%d")
        issue_time = etree.SubElement(parent_node, ns["cbc"] + "IssueTime")
        issue_time.text = self.l10n_sa_confirmation_datetime.strftime(
            "%H:%M:%S")
        move_type = "01" if self.partner_id.company_type == "company" else "02"
        if self.partner_id.country_id != self.company_id.country_id and self.partner_id.company_type == "company":
            self.position5 = True
        name_tag = (
            move_type
        ...
        type_code = etree.SubElement(
            parent_node, ns["cbc"] + "InvoiceTypeCode", name=name_tag)
        if self.move_type == "out_invoice":
            if ("debit_origin_id" in self) and self.debit_origin_id:
                # Debit note
                type_code.text = "383"
            else:
                # Sales invoice
        ...
        count_lines = etree.SubElement(
            parent_node, ns["cbc"] + "LineCountNumeric")
        count_lines.text = self._get_lines_invoice()

    def _xml_add_attachments(self, parent_node, ns):
        doc_counter = etree.SubElement(
        ...
        sha256 = self._encrypt_string(
            previous_invoice and previous_invoice.id or 0)
        pi_reference = etree.SubElement(
            parent_node, ns["cac"] + "AdditionalDocumentReference")
        ...
        binary_node = etree.SubElement(
            attach_node, ns["cbc"] + "EmbeddedDocumentBinaryObject", mimeCode="text/plain")
    ...
    def _xml_add_party_identification(self, scheme_name, party_id_text, parent_node, ns):
        if party_id_text:
            ...
            party_identification_id = etree.SubElement(
                party_identification, ns["cbc"] + "ID", schemeID=scheme_name)
            party_identification_id.text = party_id_text
        return

    @api.model
    def _xml_add_party_tax_scheme(self, commercial_partner, parent_node, ns):
        if commercial_partner.vat and commercial_partner.country_id.code == 'SA':
    ...
    def _xml_add_party(self, partner, company, node_name, parent_node, ns):
        ...
        # todo add other IDs
        pparty = partner or company
        id_attrs = pparty.additional_identification_scheme
        id = pparty.additional_identification_number if pparty.additional_identification_scheme != 'TIN' else pparty.vat
        if id_attrs and id:
            self._xml_add_party_identification(id_attrs, id, party, ns)
        # if company:
        #     self._xml_add_party_identification(company.additional_identification_scheme, company.additional_identification_number, party, ns)
        # elif partner.vat:
    ...
    def _xml_add_supplier_party(self, partner, company, parent_node, ns):
        ...
        supplier_party_root = etree.SubElement(
            parent_node, ns["cac"] + "AccountingSupplierParty")
        self._xml_add_party(
            partner,
            company,
            "Party",
    ...
    def _xml_add_customer_party(self, partner, company, parent_node, ns):
        ...
        customer_party_root = etree.SubElement(
            parent_node, ns["cac"] + "AccountingCustomerParty")
    ...
    def _xml_add_delivery(self, parent_node, ns):
        ...
        delivery_Adate = etree.SubElement(
            delivery, ns["cbc"] + "ActualDeliveryDate")
        delivery_Adate.text = self.l10n_sa_delivery_date.strftime("%Y-%m-%d")
        # delivery_Ldate = etree.SubElement(delivery, ns["cbc"] + "LatestDeliveryDate")
        # delivery_Ldate.text = self.invoice_date_due.strftime("%Y-%m-%d")
    ...
    def _xml_add_payment_means(self, payment_mode, parent_node, ns):
        ...
        if self.move_type == "out_refund" or (self.move_type == "out_invoice" and self.debit_origin_id):
            instruction_note = etree.SubElement(
                pay_means, ns["cbc"] + "InstructionNote")
            instruction_note.text = self.ref

    @api.model
    def _xml_add_billing_reference(self, parent_node, ns):
        if not self.ref:
            raise ValidationError(
                "Please, make sure the Reversal Reason is specified when confirming a Credit/Debit note")
        billing_reference = etree.SubElement(
            parent_node, ns["cac"] + "BillingReference")
        invoice_document_reference = etree.SubElement(
            billing_reference, ns["cac"] + "InvoiceDocumentReference")
        billing_reference_id = etree.SubElement(
            invoice_document_reference, ns["cbc"] + "ID")
        billing_reference_id.text = self.ref
    ...
    def _xml_add_tax_category(self, tax, parent_node, ns, node_name="TaxCategory"):
        ...
        if supplier.country_id.code == 'SA':
            if not tax or tax.amount == 0:
                tax_exemption_reason_code = etree.SubElement(
                    tax_category, ns["cbc"] + "TaxExemptionReasonCode")
                tax_exemption_reason = etree.SubElement(
                    tax_category, ns["cbc"] + "TaxExemptionReason")
                exemption_codes = dict(
                    tax._fields["exemption_reason_code"]._description_selection(self.env))
                tax_exemption_reason_code.text = tax.exemption_reason_code
                tax_exemption_reason.text = exemption_codes[tax.exemption_reason_code].split(
                    tax.exemption_reason_code)[1].lstrip()
                if tax.exemption_reason_code in TAX_EXEMPTION_CODES:
                    tax_category_id.text = "E"
                elif tax.exemption_reason_code in TAX_ZERO_RATE_CODES:
                    tax_category_id.text = "Z"
                elif tax.exemption_reason_code == "VATEX-SA-OOS":
                    tax_category_id.text = "O"
                    tax_exemption_reason_code.text = "VATEX-SA-OOS"
                    tax_exemption_reason.text = tax.free_exemption_reason
                else:
                    tax_category_id.text = "O"
                    tax_exemption_reason_code.text = "Not subject to VAT"
                    tax_exemption_reason.text = "Not subject to VAT"
    ...
    def _xml_add_allowance_charge(self, discount, price_subtotal, parent_node, ns):
        ...
        allowance_charge = etree.SubElement(
            parent_node, ns["cac"] + "AllowanceCharge")
        charge_indicator = etree.SubElement(
            allowance_charge, ns["cbc"] + "ChargeIndicator")
        charge_indicator.text = "false"
        charge_indicator = etree.SubElement(
            allowance_charge, ns["cbc"] + "AllowanceChargeReasonCode")
        charge_indicator.text = "95"
        charge_indicator = etree.SubElement(
            allowance_charge, ns["cbc"] + "AllowanceChargeReason")
        charge_indicator.text = "Discount"
        factor_numeric = etree.SubElement(
            allowance_charge, ns["cbc"] + "MultiplierFactorNumeric")
        factor_numeric.text = "%0.*f" % (2, discount)
        if discount == 100.0:
            amount = 0.0
        else:
            amount = price_subtotal * discount / (100.0 - discount)
        base_amount = price_subtotal + amount
        amount_node = etree.SubElement(
            allowance_charge, ns["cbc"] + "Amount", currencyID=cur_name)
        amount_node.text = "%0.*f" % (2, amount)
        base_amount_node = etree.SubElement(
            allowance_charge, ns["cbc"] + "BaseAmount", currencyID=cur_name)
        base_amount_node.text = "%0.*f" % (2, base_amount)
    ...
    def _xml_add_tax_total(self, simplified, xml_root, ns):
        ...
        tax_amount_node = etree.SubElement(
            tax_total_node, ns["cbc"] + "TaxAmount", currencyID=currency_id.name)
        prec = currency_id.decimal_places
        # prec = prec if prec <= 2 else 2
        amount_tax_signed = self.amount_tax_signed
        downpayment_lines = False if self._is_downpayment(
        ) else self.line_ids.filtered(lambda l: l._get_downpayment_lines())
        prepayment_move_ids = []
        if downpayment_lines:
            for dline in downpayment_lines:
                prepayment_move_ids.append(dline.sale_line_ids.invoice_lines.move_id.filtered(
                    lambda m: m._is_downpayment()))
            damount_tax = 0.0
            dtax_lines = self.env["account.move.line"]
            dsup_tax_lines = self.env["account.move.line"]
            for l in prepayment_move_ids:
                amount_tax_signed += l.amount_tax_signed
                damount_tax += l.amount_tax
                dtax_lines |= l.line_ids.filtered(
                    lambda line: line.tax_line_id)
                dsup_tax_lines |= l.invoice_line_ids.filtered(
                    lambda line: (l.currency_id.is_zero(sum([tax.amount for tax in line.tax_ids])) or l.currency_id.is_zero(
                        line.price_total)) and not line.display_type
                )
        tax_amount_node.text = "%0.*f" % (2, abs(amount_tax_signed))
        if simplified == False:
            amount_tax = self.amount_tax
            tax_lines = self.line_ids.filtered(lambda line: line.tax_line_id)
            sup_tax_lines = self.invoice_line_ids.filtered(
                lambda line: (self.currency_id.is_zero(sum([tax.amount for tax in line.tax_ids])) or self.currency_id.is_zero(
                    line.price_total)) and not line.display_type
            )
            if downpayment_lines:
                amount_tax += damount_tax
                tax_lines |= dtax_lines
                sup_tax_lines |= dsup_tax_lines
            tax_amount_node.text = "%0.*f" % (2, amount_tax)
            res = {}
            for line in tax_lines:
                tax_base_amount_currency = line.move_id.company_currency_id._convert(
                    line.tax_base_amount, line.move_id.currency_id, line.move_id.company_id, line.move_id.date
                )
                res.setdefault(
                    line.tax_line_id,
                    {"base": 0.0, "amount": 0.0},
                )
                res[line.tax_line_id]["base"] += tax_base_amount_currency
                res[line.tax_line_id]["amount"] += line.price_subtotal
            for line in sup_tax_lines:
                res.setdefault(
                    line.tax_ids[0],
                    {"base": 0.0, "amount": 0.0},
                )
                res[line.tax_ids[0]]["base"] += line.price_subtotal
            res = sorted(res.items(), key=lambda l: l[0].sequence)
            for _group, amounts in res:
                self._xml_add_tax_subtotal(
                    amounts["base"],
                    amounts["amount"],
                    _group,
                    currency_id.name,
    ...
    def _xml_add_line_prepayment_vals(self, line, parent_node, ns):
        move_lines = self.env['account.move.line']
        if hasattr(move_lines, "sale_line_ids"):
            if not line.move_id._is_downpayment() and line.sale_line_ids and all(sale_line.is_downpayment for sale_line in line.sale_line_ids):
                prepayment_move_id = line.sale_line_ids.invoice_lines.move_id.filtered(
                    lambda m: m._is_downpayment())
                doc_ref_node = etree.SubElement(
                    parent_node, ns["cac"] + "DocumentReference")
                doc_ref_id = etree.SubElement(doc_ref_node, ns["cbc"] + "ID")
                doc_ref_id.text = prepayment_move_id.name
                issueDate = fields.Datetime.context_timestamp(self.with_context(tz='Asia/Riyadh'),
                                                              prepayment_move_id.l10n_sa_confirmation_datetime)
                doc_ref_issueDate = etree.SubElement(
                    doc_ref_node, ns["cbc"] + "IssueDate")
                doc_ref_issueDate.text = issueDate.strftime('%Y-%m-%d')
                doc_ref_issueTime = etree.SubElement(
                    doc_ref_node, ns["cbc"] + "IssueTime")
                doc_ref_issueTime.text = issueDate.strftime('%H:%M:%S')
                doc_ref_tc = etree.SubElement(
                    doc_ref_node, ns["cbc"] + "DocumentTypeCode")
                doc_ref_tc.text = "386"
        else:
            return False
    ...
    def _xml_add_invoice_line(self, parent_node, iline, line_number, ns):
        ...
        if iline.product_uom_id and iline.product_uom_id._get_unece_code():
            uom_unece_code = iline.product_uom_id._get_unece_code()
        if uom_unece_code:
        ...
        qty = abs(iline.quantity)
        price_subtotal = iline.price_subtotal
        if not iline.move_id._is_downpayment() and iline._get_downpayment_lines():
            price_subtotal = 0.0
            qty = 0.0
        quantity.text = "%0.*f" % (qty_precision, qty)
        line_amount = etree.SubElement(
            line_root, ns["cbc"] + "LineExtensionAmount", currencyID=cur_name)
        line_amount.text = "%0.*f" % (2, price_subtotal)
        self._xml_add_line_prepayment_vals(iline, line_root, ns)
        if iline.discount:
            self._xml_add_allowance_charge(
                iline.discount, iline.price_subtotal, line_root, ns)
        ...
        price_unit = 0.0
        if not float_is_zero(qty, precision_digits=qty_precision) and (iline.move_id._is_downpayment() or not iline._get_downpayment_lines()):
            if iline.discount == 100.0:
                glo_price_subtotal = 0.0
            else:
                glo_price_subtotal = iline.price_subtotal / \
                    (1.0 - (iline.discount or 0.0) / 100.0)
            glo_price_unit = glo_price_subtotal / iline.quantity if iline.quantity else 0.0
            price_unit = float_round(
                glo_price_unit, precision_digits=price_precision)
    ...
    def _xml_add_invoice_line_tax_total(self, iline, parent_node, ns):
        ...
        tax_total = float_round(
            res_taxes["total_included"] - res_taxes["total_excluded"], precision_digits=prec)
        rounding_amount = res_taxes["total_included"]
        if not iline.move_id._is_downpayment() and iline._get_downpayment_lines():
            tax_total = 0
            rounding_amount = 0
        tax_amount_node = etree.SubElement(
        ...
        rounding_amount_node.text = "%0.*f" % (2, rounding_amount)
        if not iline.move_id._is_downpayment() and iline._get_downpayment_lines():
            prepayment_move_id = iline.sale_line_ids.invoice_lines.move_id.filtered(
                lambda m: m._is_downpayment())
            tax_lines = prepayment_move_id.line_ids.filtered(
                lambda line: line.tax_line_id)
            sup_tax_lines = prepayment_move_id.invoice_line_ids.filtered(
                lambda line: (prepayment_move_id.currency_id.is_zero(sum(
                    [tax.amount for tax in line.tax_ids])) or prepayment_move_id.currency_id.is_zero(line.price_total)) and not line.display_type
            )
            res = {}
            for line in tax_lines:
                tax_base_amount_currency = prepayment_move_id.company_currency_id._convert(
                    line.tax_base_amount, prepayment_move_id.currency_id, prepayment_move_id.company_id, prepayment_move_id.date
                )
                res.setdefault(
                    line.tax_line_id,
                    {"base": tax_base_amount_currency,
            ...
            for _group, amounts in res:
                self._xml_add_tax_subtotal(
                    ...
                    amounts["amount"],
                    _group,
                    prepayment_move_id.currency_id.name,
                    tax_total_node,
                    ns,
                )
    ...
    def generate_invoice_xml_etree(self, ProfileID="reporting:1.0"):
        ...
        line_number = 0
        for iline in self.invoice_line_ids.filtered(lambda line: not line.display_type):
            line_number += 1
    ...
    def _post(self, soft=True):
        ...
        for record in self:
            if record.country_code == "SA" and record.move_type in ("out_invoice", "out_refund"):
                ...
                # if not all(line.tax_ids for line in record.invoice_line_ids.filtered(lambda line: not line.display_type)):
                #     if not self.company_id.sudo().pem_private_key or not self.compnay_id.digital_certificate:
        ...
        zatca_auto_send = self.env["ir.config_parameter"].sudo(
        ).get_param("l10n_sa_e-invoice.zatca_auto_send")
        if zatca_auto_send:
    ...
    def attach_generated_xml(self):
        ...
        if not self.xml_file:
            attach = self.env["ir.attachment"].create(
                {
                    "name": "E-Invoice-%s.xml" % self.name,
                    "res_id": self.id,
