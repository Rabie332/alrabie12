# See LICENSE file for full copyright and licensing details.

{
    "name": "Hotel Management",
    "version": "14.0.1.0.0",
    "author": "Eng. Fares",
    "category": "Hotel Management",
    "website": "https://github.com/OCA/vertical-hotel",
    "depends": ["sale_stock", "account", "sale_management", 'base', 'base_user_role', 'hr'],
    "license": "AGPL-3",
    "summary": "Hotel Management to Manage Folio and Hotel Configuration",
    "demo": ["demo/hotel_data.xml"],
    "data": [
        "security/hotel_security.xml",
        "security/ir.model.access.csv",
        "data/hotel_sequence.xml",
        "report/report_view.xml",
        "report/hotel_folio_report_template.xml",
        "report/hotel_lost_report_template.xml",
        "report/hotel_verifiacation_report_template.xml",
        "views/hotel_folio.xml",
        "views/hotel_room.xml",
        "views/hotel_room_amenities.xml",
        "views/hotel_room_type.xml",
        "views/hotel_service_type.xml",
        "views/hotel_services.xml",
        "views/product_product.xml",
        "views/res_company.xml",
        "views/actions.xml",
        "views/menus.xml",
        "views/price_setting_views.xml",
        "views/hotel_lost_found_views.xml",
        "views/hotel_verification.xml",
        "wizard/hotel_wizard.xml",
        "wizard/hotel_lost_found_wizard.xml",
        "wizard/hotel_verification_wizard.xml",
        
    ],
    "css": ["static/src/css/room_kanban.css"],
    "external_dependencies": {"python": ["dateutil"]},
    "images": ["static/description/Hotel.png"],
    "application": True,
    "installable": True,
}
