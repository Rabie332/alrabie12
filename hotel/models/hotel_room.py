...
class HotelRoom(models.Model):
    ...
    is_withheld = fields.Boolean(string="Is Withheld")
    insurance = fields.Float(string="Insurance")
    monthly_price = fields.Float(string="Monthly price")
    hourly_price = fields.Monetary(string="Hourly price")
    is_clean = fields.Boolean(string="Clean", tracking=1, copy=False, default=True)
    ...
    service_status = fields.Selection(
        ...
        selection=[("clean", "Clean"), ("dirty", "Dirty")],
        store=1,
        compute="_compute_service_status",
    )
    minimum_monthly_price = fields.Float(string="Minimum Monthly price")
    minimum_hourly_price = fields.Float(string="Minimum Hourly price")
    minimum_daily_price = fields.Float(string="Minimum Daily price")

    @api.depends("is_clean")
    def _compute_service_status(self):
        for room in self:
            room.service_status = "clean"
            if not room.is_clean:
                room.service_status = "dirty"
    ...
    def create(self, vals):
        ...
        if room:
            room.product_id.invoice_policy = "order"
        return room
    ...
    def button_available(self):
        """Change state room to available."""
        self.status = "available"
        self.isroom = True
    ...
    def toggle_cleaned(self):
        """Publish in website."""
        self.write({"is_clean": not self.is_clean})
        return True
