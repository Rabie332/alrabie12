...
class HotelFolio(models.Model):
    ...
    def action_folio_cancel(self):
        """
        Cancel folio without change room to available
        """
        for folio in self:
            folio.invoice_ids.button_cancel()
            return folio.order_id.action_cancel()
    ...
    def action_cancel_draft(self):
        ...
        order_line_recs.write({"state": "draft", "invoice_lines": [(6, 0, [])]})

    def create_invoices(self):
        """Create invoice."""
        context = {
            "active_model": "hotel.folio",
            "active_ids": self.ids,
            "active_id": self.id,
            "open_invoices": True,
            "mail_notrack": True,
        ...
        for line in self:
            if line.order_line_id:
                ...
                folio_room_lines = self.env["folio.room.line"].search(
                    [("folio_id", "=", line.folio_id.id), ("room_id", "in", rooms.ids)]
                )
