odoo.define("ks_dashboard_ninja.ks_ylabels", function (require) {
    ...
    var KsYLabels = AbstractField.extend({
        ...
        _render: function () {
            ...
            if (
                field.ks_query_result &&
                field.ks_dashboard_item_type !== "ks_kpi"
            ) {
                var ks_query_result = JSON.parse(field.ks_query_result);
                if (
                    field.ks_dashboard_item_type !== "ks_kpi" &&
                    ks_query_result.header.length
                ) {
                    if (
                        field.ks_dashboard_item_type !== "ks_list_view" &&
                        field.ks_dashboard_item_type !== "ks_kpi" &&
                        field.ks_dashboard_item_type !== "ks_tile"
                    ) {
                        self.ks_check_for_labels();
                        var $view = $(
                            QWeb.render("ks_y_label_table", {
                                label_rows: self.ks_columns,
                                chart_type: self.ks_rows_chart_type,
                                mode: self.mode,
                                ks_is_group_column:
                                    ks_query_result.ks_is_group_column,
                            })
                        );
                        ...
                        self.ks_rows_keys.forEach(function (key) {
                            $view
                                .find("#" + key)
                                .val(self.ks_rows_chart_type[key]);
                        });
        ...
        ks_check_for_labels: function () {
            ...
            if (self.value) {
                ...
                Object.keys(ks_columns).forEach(function (key) {
                    ...
                    if (
                        self.recordData.ks_dashboard_item_type ===
                        "ks_bar_chart"
                    ) {
                        var chart_type_temp =
                            self.recordData.ks_dashboard_item_type.split(
                                "_"
                            )[1];
                        if (chart_type_temp !== chart_type) {
                            chart_type = chart_type_temp;
                        }
                        ks_columns[key].chart_type[chart_type] =
                            self.ks_title(chart_type);
                        if (chart_type === "bar") {
                ...
                query_result.header.forEach(function (key) {
                    for (var i = 0; i < query_result.header.length; i++) {
                        if (
                            query_result.type_code[
                                query_result.header.indexOf(key)
                            ] !== "numeric"
                        ) {
                            continue;
                        }
                        if (
                            ...
                            ks_row.chart_type[chart_type] =
                                self.ks_title(chart_type);
                            if (chart_type === "bar")
                                ks_row.chart_type.line = "Line";
                            ks_row.group = " ";
        ...
        ks_title: function (str) {
            ...
            for (var i = 0; i < split_str.length; i++) {
                split_str[i] =
                    split_str[i].charAt(0).toUpperCase() +
                    split_str[i].substring(1);
                str = split_str.join(" ");
            }
            return str;
        },
        ...
        ks_group_input_click: function (e) {
            ...
            self.ks_columns[e.currentTarget.id].group =
