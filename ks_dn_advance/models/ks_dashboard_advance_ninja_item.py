...
class KsDashboardNinjaItemAdvance(models.Model):
    ...
    ks_list_view_layout = fields.Selection(
        [
        ...
        string="List View Layout",
        default="layout_1",
    )

    ks_is_date_ranges = fields.Boolean(
        "Date Ranges",
    ...
    def _ksGetKpiData(self, domain1=[], domain2=[]):
        ...
        if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type == "ks_kpi":
            if rec.ks_data_calculation_type != "query":
                return super(KsDashboardNinjaItemAdvance, self)._ksGetKpiData(
                ...
                ks_data_list = [
                    {"record_field": False, "record_data": 0, "model": False}
                ]
    ...
    def _ks_get_chart_data(self, domain=[]):
        for rec in self:
            if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type not in [
                "ks_tile",
                "ks_list_view",
                "ks_kpi",
            ]:
                if rec.ks_data_calculation_type != "query":
                    return super(KsDashboardNinjaItemAdvance, self)._ks_get_chart_data(
                        domain
                    )
                else:
                        ...
                        if records:
                            ...
                            if rec.ks_xlabels and rec.ks_ylabels:
                                ...
                                for y_axis in ks_yaxis.keys():
                                    data_row = {
                                        "data": [],
                                        "label": ks_yaxis[y_axis]["measure"],
                                    }

                                    if rec.ks_dashboard_item_type in [
                                        "ks_bar_chart",
                                        ...
                                        if rec.ks_bar_chart_stacked:
                                            data_row["stack"] = ks_yaxis[y_axis][
                                                "group"
                                            ]
                                        if ks_yaxis[y_axis]["chart_type"] == "line":
                                            y_labels.insert(0, y_axis)
                                            ks_chart_data["datasets"].insert(
                                                0, data_row
                                            )
                                        else:
                                ...
                                for res in records.get("records"):
                                    ...
                                    if res.get(rec.ks_xlabels, False):
                                        ...
                                        for y_axis in y_labels:
                                            ks_chart_data["datasets"][counter][
                                                "data"
                                            ].append(res[y_axis])
                                            counter += 1
    ...
    def ks_get_list_view_data(self):
        for rec in self:
            if (
                ...
                if rec.ks_data_calculation_type != "query":
                    return super(
                        KsDashboardNinjaItemAdvance, self
                    ).ks_get_list_view_data()
                elif rec.ks_data_calculation_type == "query" and rec.ks_query_result:
    ...
    def _ksGetListViewData(self, domain=[]):
        ...
        if (
            ...
            if rec.ks_data_calculation_type != "query":
                return super(KsDashboardNinjaItemAdvance, self)._ksGetListViewData(
                    domain
                )
            elif rec.ks_data_calculation_type == "query" and rec.ks_query_result:
                ks_list_view_data = rec.ks_format_query_result(rec.ks_query_result)
    ...
    def ks_run_query(self):
        ...
        if self.ks_is_date_ranges:
            ...
            if self.ks_custom_query and (
                ("%(ks_start_date)" in self.ks_custom_query)
                or ("%(ks_end_date)" in self.ks_custom_query)
            ):
                ks_type = "date"
                ks_start_date = "ks_start_date"
                ks_end_date = "ks_end_date"
            if self.ks_custom_query and (
                ...
                or ("%(ks_start_datetime)" in self.ks_custom_query)
            ):
                ks_type = "datetime"
            ...
            if self._context.get("ksDateFilterSelection", False):
                ...
                if ksDateFilterSelection not in ["l_custom", "l_none"]:
                    ks_get_date_ranges = ks_get_date(
                        ksDateFilterSelection, self, ks_type
                    )
        ...
        for rec in self:
            ...
            with api.Environment.manage():
                ...
                if (
                    ...
                    and rec.ks_dashboard_item_type not in ["ks_tile", "ks_kpi"]
                    and rec.ks_custom_query
                ):
                    if ks_query and "{#MYCOMPANY}" in ks_query:
                        ks_query = ks_query.replace(
                    ...
                    if rec.ks_dashboard_item_type == "ks_list_view":
                        rec.ks_query_result = rec.ks_get_list_query_result(
                            ks_query, selected_start_date, selected_end_date
                        )
                    else:
                        with api.Environment.manage():
                            try:
                                ...
                                if rec.ks_is_date_ranges:
                                    ...
                                    if selected_end_date or selected_start_date:
                                        start_date = (
                                            ...
                                            - relativedelta.relativedelta(years=1000)
                                        )
                                        end_date = (
                                            selected_end_date
                                            if selected_end_date
                                            else selected_start_date
                                            + relativedelta.relativedelta(years=1000)
                                        )

                                    new_env.execute(
                                        "with ks_chart_query as ("
                                        + ks_query
                                        + ")"
                                    ...
                                    header_rec = [
                                        col.name for col in new_env.description
                                    ]
                                    result = new_env.dictfetchall()
                                    if result:
                                        for header_key in header_rec:
                                            if (
                                                type(result[0][header_key]).__name__
                                                == "float"
                                                or type(result[0][header_key]).__name__
                                                == "int"
                                            ):
                                ...
                                if records:
                                    ...
                                    for header_key in header:
                                        if (
                                            type(records[0][header_key]).__name__
                                            == "float"
                                            or type(records[0][header_key]).__name__
                                            == "int"
                                        ):
                                            type_code.append("numeric")
                                        else:
                ...
                elif rec.ks_dashboard_item_type in ["ks_kpi"] and rec.ks_custom_query:
                    if rec.ks_is_date_ranges and not (
                    ...
                    ks_query_result = rec.ks_get_kpi_result(
                        ks_query,
                        selected_start_date,
                        selected_end_date,
                        ks_start_date=ks_start_date,
    ...
    def ks_get_kpi_result(
        ...
        conn = sql_db.db_connect(self.env.cr.dbname)
        new_env = api.Environment(conn.cursor(), self.env.uid, self.env.context)
        if ks_query and "{#MYCOMPANY}" in ks_query:
            ks_query = ks_query.replace(
        ...
        if self.ks_is_date_ranges:
            ...
            new_env.cr.execute(
                "with ks_list_query as ("
                + ks_query
                + ")"
                + "select * from ks_list_query",
                {ks_start_date: str(start_date), ks_end_date: str(end_date)},
            )
        else:
            new_env.cr.execute(
                "with ks_list_query as ("
    ...
    def ks_validate_kpi_query(
        self, ks_query, start_date, end_date, ks_start_date=False, ks_end_date=False
    ):
        try:
            conn = sql_db.db_connect(self.env.cr.dbname)
            new_env = api.Environment(conn.cursor(), self.env.uid, self.env.context)
            if self.ks_is_date_ranges:
                new_env.cr.execute(
                    ...
                    + ks_query
                    + ")"
                    + "select * from ks_list_query limit 5",
                    {ks_start_date: str(start_date), ks_end_date: str(end_date)},
                )
            else:
                new_env.cr.execute(
            ...
            try:
                ...
                if self.ks_is_date_ranges:
                    ...
                    if result:
                        for header_key in header_rec:
                            if (
                                type(result[0][header_key]).__name__ == "float"
                                or type(result[0][header_key]).__name__ == "int"
                            ):
                                type_code.append("numeric")
                            else:
                                type_code.append("string")
                ...
                if records:
                    ...
                    for header_key in header:
                        if (
                            type(records[0][header_key]).__name__ == "float"
                            or type(records[0][header_key]).__name__ == "int"
                        ):
                            type_code.append("numeric")
                        else:
                            type_code.append("string")
    ...
    def ks_get_next_offset(self, ks_item_id, offset, item_domain=[]):
        ...
        if self.ks_data_calculation_type == "custom":
            return super(KsDashboardNinjaItemAdvance, self).ks_get_next_offset(
                ks_item_id, offset, item_domain
            )
        else:
    ...
    def ks_get_list_data_orderby_extend(self, domain={}):
        ...
        self._context.get("field", False)
        sort_order = self._context.get("sort_order", False)
        self._context.get("offset", 0)
            ...
            if ks_date_filter_selection == "l_custom":
                self = self.with_context(
                    ksDateFilterStartDate=fields.datetime.strptime(
                        self._context["ksDateFilterStartDate"], "%Y-%m-%d %H:%M:%S"
                    )
                )
                self = self.with_context(
                    ksDateFilterEndDate=fields.datetime.strptime(
                        self._context["ksDateFilterEndDate"], "%Y-%m-%d %H:%M:%S"
                    )
                )
                self = self.with_context(ksIsDefultCustomDateFilter=False)
        else:
            ks_date_filter_selection = (
                self.ks_dashboard_ninja_board_id.ks_date_filter_selection
            )
            self = self.with_context(
                ksDateFilterStartDate=self.ks_dashboard_ninja_board_id.ks_dashboard_start_date
            )
            self = self.with_context(
                ksDateFilterEndDate=self.ks_dashboard_ninja_board_id.ks_dashboard_end_date
            )
        ...
        list_view_data = self.get_list_view_record(
            orderby,
            sort_order,
            ks_domain,
