...
class ReportFinancial(models.AbstractModel):
    ...
    def get_account_lines(self, data):  # noqa: C901
        lines = []
        account_lines = []
        account_report = (
            self.env["account.financial.report"]
            .with_context(lang=self.env.user.lang)
            .search([("id", "=", data["account_report_id"][0])])
        )
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # lang = data["account_financial_report_lang"]
        docs = self.env[model].browse(self.env.context.get("active_id"))
        used_context = data.get("form").get("used_context")
        if used_context and used_context.get("analytic_account_ids"):
            analytic_accounts = self.env["account.analytic.account"].browse(
                used_context.get("analytic_account_ids")
            )
            data["form"]["used_context"]["analytic_account_ids"] = analytic_accounts
