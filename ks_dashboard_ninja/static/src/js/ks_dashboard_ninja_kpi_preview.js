odoo.define(
    "ks_dashboard_ninja_list.ks_dashboard_kpi_preview",
    function (require) {
        "use strict";
        ...
        var KsKpiPreview = AbstractField.extend({
            ...
            _render: function () {
                ...
                if (
                    ...
                    if (!this.recordData.ks_model_id_2) {
                        ...
                        !(this.recordData.ks_record_count_type_2 === "count") &&
                        !(this.recordData.ks_record_count_type === "count")
                    ) {
                        if (
                            this.recordData.ks_record_field_2 &&
                            this.recordData.ks_record_field
                        ) {
                            this.renderKpi();
                        } else {
            ...
            ksSum: function (
                ...
                var target_deviation =
                    target_1 - count > 0
                        ...
                        : Math.round(
                              (Math.abs(target_1 - count) / target_1) * 100
                          );
                ...
                $kpi_preview = $(
                    Qweb.render("ks_kpi_preview_template_2", item_info)
                );
            ...
            renderKpi: function () {
                ...
                var ks_rgba_background_color = self._get_rgba_format(
                    field.ks_background_color
                );
                var ks_rgba_font_color = self._get_rgba_format(
                    field.ks_font_color
                );
                ...
                if (
                    ...
                    ks_valid_date_selection.indexOf(
                        field.ks_date_filter_selection
                    ) >= 0
                ) {
                ...
                var ks_rgba_icon_color = self._get_rgba_format(
                ...
                if (!kpi_data[1]) {
                    if (target_view === "Number" || !field.ks_goal_enable) {
                        $kpi_preview = $(
                            Qweb.render("ks_kpi_preview_template", item_info)
                        );
                    } else if (
                        target_view === "Progress Bar" &&
                        field.ks_goal_enable
                    ) {
                        $kpi_preview = $(
                            Qweb.render("ks_kpi_preview_template_3", item_info)
                        );
                        $kpi_preview
                            .find("#ks_progressbar")
                    ...
                    switch (field.ks_data_comparison) {
                        case "None":
                            if (field.ks_multiplier_active) {
                                ...
                                var count =
                                    String(self.ksNumFormatter(count_1, 1)) +
                                    "/" +
                                    String(self.ksNumFormatter(count_2, 1));
                                item_info.count =
                                    String(
                                        KsGlobalFunction._onKsGlobalFormatter(
                                            count_1,
