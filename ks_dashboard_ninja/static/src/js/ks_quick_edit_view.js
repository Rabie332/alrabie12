odoo.define("ks_dashboard_ninja.quick_edit_view", function (require) {
    ...
    var QuickEditView = Widget.extend({
        ...
        _ksCreateController: function () {
            ...
            self.context.form_view_ref =
                "ks_dashboard_ninja.item_quick_edit_form_view";
            ...
            return $.when(def).then(function (fieldsViews) {
                ...
                return $.when(def2).then(function (controller) {
                    ...
                    self.controller._confirmChange =
                        self._confirmChange.bind(self);
                });
            });
        },
        ...
        _confirmChange: function (id, fields, e) {
            if (e.name === "discard_changes" && e.target.reset) {
                ...
                return e.target.reset(
                    this.controller.model.get(e.target.dataPointID),
                    e,
                    true
                );
            }
        ...
        ks_Update_item: function () {
            ...
            if (ksChanges.ks_list_view_data)
                self.item.ks_list_view_data = ksChanges.ks_list_view_data;

            if (ksChanges.ks_chart_data)
                self.item.ks_chart_data = ksChanges.ks_chart_data;

            if (ksChanges.ks_kpi_data)
                self.item.ks_kpi_data = ksChanges.ks_kpi_data;

            if (ksChanges.ks_list_view_type)
                self.item.ks_list_view_type = ksChanges.ks_list_view_type;

            if (ksChanges.ks_chart_item_color)
        ...
        ksOnQuickEditViewAction: function (e) {
            ...
            if (e.currentTarget.dataset.ksItemAction === "saveItemInfo") {
                ...
                e.currentTarget.dataset.ksItemAction === "fullItemInfo"
            ) {
