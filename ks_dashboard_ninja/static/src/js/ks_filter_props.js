odoo.define("ks_dashboard_ninja.ks_dashboard_filter", function (require) {
    ...
    return KsFilterProps.include({
        events: _.extend({}, KsFilterProps.prototype.events, {
            "hide.bs.dropdown .ks_dn_selection_box > div":
                "onKsDnFilterBoxContainerHide",
            "click .dn_filter_click_event_selector":
                "onKsDnDynamicFilterSelect",
            "click .ks_custom_filter_add_condition":
                "ksOnCustomFilterConditionAdd",
            "click .ks_custom_filter_section_delete":
                "ksOnCustomFilterConditionRemove",
            "click .ks_dn_filter_apply": "ksOnCustomFilterApply",
            "click .ks_dn_filter_remove_event":
                "ksOnRemoveFilterFromSearchPanel",
            "change .ks_custom_filter_field_selector":
                "ksOnCustomFilterFieldSelect",
            "change .ks_custom_filter_field_selector":
                "ksOnCustomFilterFieldSelect",
            "change .ks_operator_option_selector":
                "ksOnCustomFilterOperatorSelect",
            "change .ks_operator_option_selector":
                "ksOnCustomFilterOperatorSelect",
        }),
        ...
        onKsDnFilterBoxContainerHide: function (ev) {
            if (
                ev.clickEvent &&
                ev.clickEvent.target &&
                $(ev.clickEvent.target).parents(
                    ".ks_dn_filter_dropdown_container"
                ).length
            ) {
                return false;
            }
        ...
        _ksAppendDynamicFilter: function (filterId) {
            ...
            this.ks_dashboard_data.ks_dashboard_pre_domain_filter[
                filterId
            ].active = true;

            var action = "add_dynamic_filter";
            var categ =
                this.ks_dashboard_data.ks_dashboard_pre_domain_filter[filterId]
                    .categ;
            var params = {
                model: this.ks_dashboard_data.ks_dashboard_pre_domain_filter[
                    filterId
                ].model,
                model_name:
                    this.ks_dashboard_data.ks_dashboard_pre_domain_filter[
        ...
        _ksUpdateAddDomainIndexData: function (action, categ, params) {
            ...
            var filters_to_update = _(
                this.ks_dashboard_data.ks_dashboard_pre_domain_filter
            )
                .values()
                .filter((x) => {
                    return x.active === true && x.categ === categ;
                });
            var domain_data =
                self.ks_dashboard_data.ks_dashboard_domain_data[model];
            if (domain_data) {
                var domain_index = _(domain_data.ks_domain_index_data).find(
                    (x) => {
                        return x.categ === categ;
                    }
                );
                if (domain_index) {
                    domain_index.domain = [];
                    ...
                    _(filters_to_update).each((x) => {
                        if (domain_index.domain.length > 0)
                            domain_index.domain.unshift("|");
                        domain_index.domain = domain_index.domain.concat(
                            x.domain
                        );
                ...
                var $filter_container = $(
                    QWeb.render("ks_dn_filter_section_container_template", {
                        ks_domain_data: domain_data,
                        ks_model: model,
                    })
                );
                $(
                    '.ks_dn_filter_section_container[data-ks-model-selector="' +
                        model +
                        '"]'
                ).replaceWith($filter_container);
            } else {
                var domain_index = {
                    categ: categ,
                    domain: [],
                ...
                domain_data = {
                    ...
                    item_ids:
                        self.ks_dashboard_data.ks_model_item_relation[model],
            ...
            domain_data.domain = self._ksMakeDomainFromDomainIndex(
                domain_data.ks_domain_index_data
            );

            _(domain_data.item_ids).each((x) => {
                self.ksFetchUpdateItem(x);
            });
        },
        ...
        _ksUpdateRemoveDomainIndexData: function (action, categ, params) {
            ...
            var domain_data =
                self.ks_dashboard_data.ks_dashboard_domain_data[model];
            var domain_index = _(domain_data.ks_domain_index_data).find((x) => {
                return x.categ === categ;
            });

            if (filters_to_update.length < 1) {
                if (domain_data.ks_domain_index_data.length > 1) {
                    domain_data.ks_domain_index_data.splice(
                        domain_data.ks_domain_index_data.indexOf(domain_index),
                        1
                    );
                    $(
                        '.o_searchview_facet[data-ks-categ="' + categ + '"]'
                    ).remove();
                } else {
                    $(
                        '.ks_dn_filter_section_container[data-ks-model-selector="' +
                    ...
                    delete self.ks_dashboard_data.ks_dashboard_domain_data[
                        model
                    ];
                    if (
                        !_(
                        ...
                        $(".ks_dn_filter_applied_container").addClass(
                            "ks_hide"
                        );
                    }
                }
        ...
        ksGetParamsForItemFetch: function (item_id) {
            var self = this;
            var model1 =
                self.ks_dashboard_data.ks_item_model_relation[item_id][0];
            var model2 =
                self.ks_dashboard_data.ks_item_model_relation[item_id][1];

            if (model1 in self.ks_dashboard_data.ks_model_item_relation) {
                if (
                    self.ks_dashboard_data.ks_model_item_relation[
                        model1
                    ].indexOf(item_id) < 0
                )
                    self.ks_dashboard_data.ks_model_item_relation[model1].push(
                        item_id
                    );
            } else {
                self.ks_dashboard_data.ks_model_item_relation[model1] = [
                    item_id,
                ];
            }
            ...
            if (model2 in self.ks_dashboard_data.ks_model_item_relation) {
                if (
                    ...
                    self.ks_dashboard_data.ks_model_item_relation[model2].push(
                        item_id
                    );
            } else {
                self.ks_dashboard_data.ks_model_item_relation[model2] = [
                    item_id,
                ];
            }

            var ks_domain_1 =
                (self.ks_dashboard_data.ks_dashboard_domain_data[model1] &&
                    self.ks_dashboard_data.ks_dashboard_domain_data[model1]
                        .domain) ||
                [];
            var ks_domain_2 =
                (self.ks_dashboard_data.ks_dashboard_domain_data[model2] &&
                    self.ks_dashboard_data.ks_dashboard_domain_data[model2]
                        .domain) ||
                [];
        ...
        ks_fetch_items_data: function () {
            ...
            return this._super.apply(this, arguments).then(function () {
                if (self.ks_dashboard_data.ks_dashboard_domain_data)
                    self.ks_init_domain_data_index();
            });
        },
            ...
            var to_insert = _(
                ...
                .filter((x) => {
                    return (
                        x.type === "filter" &&
                        x.active &&
                        self.ks_dashboard_data.ks_dashboard_domain_data[x.model]
                            .ks_domain_index_data.length === 0
                    );
                });
            _(to_insert).each((x) => {
                if (x.categ in temp_data) {
                    temp_data[x.categ].domain = temp_data[
                        x.categ
                    ].domain.concat(x.domain);
                    temp_data[x.categ].label = temp_data[x.categ].label.concat(
                        x.name
                    );
                } else {
                    temp_data[x.categ] = {
        ...
        ksRenderDashboard: function () {
            ...
            if (
                Object.values(
                    self.ks_dashboard_data.ks_dashboard_custom_domain_filter
                ).length > 0
            )
        ...
        ks_render_custom_filter: function (show_remove_option) {
            var $container = $(
                QWeb.render("ks_dn_custom_filter_input_container", {
                    ks_dashboard_custom_domain_filter: Object.values(
                        this.ks_dashboard_data.ks_dashboard_custom_domain_filter
                    ),
                    show_remove_option: show_remove_option,
                })
            );

            var first_field_select = Object.values(
                this.ks_dashboard_data.ks_dashboard_custom_domain_filter
            )[0];
            var field_type = first_field_select.field_type;
            var operators =
                this.ksCustomFilterData.OPERATORS[
                    this.ksCustomFilterData.FIELD_TYPES[field_type]
                ];
            var operator_type = operators[0];
            var $operator_input = $(
                QWeb.render("ks_dn_custom_domain_input_operator", {
                    operators: operators,
                })
            );
            $container.append($operator_input);

            var $value_input = this._ksRenderCustomFilterInputSection(
                operator_type,
                this.ksCustomFilterData.FIELD_TYPES[field_type],
        ...
        _ksRenderCustomFilterInputSection: function (
            operator_type,
            field_type,
            special_data
        ) {
            ...
            switch (field_type) {
                ...
                case "selection":
                    ...
                    $value_input = $(
                        QWeb.render("ks_dn_custom_domain_input_selection", {
                ...
                case "datetime":
                    if ("value" in operator_type) return false;
                    $value_input = this._ksRenderDateTimeFilterInput(
                        operator_type,
                        field_type
                    );
                    break;
                ...
                case "number":
                    if ("value" in operator_type) return false;
                    $value_input = $(
                        QWeb.render("ks_dn_custom_domain_input_text", {})
                    );
                    break;
                default:
                    return;
        ...
        _ksRenderDateTimeFilterInput: function (operator, field_type) {
            var $value_container = $(
                QWeb.render("ks_dn_custom_domain_input_date")
            );
            switch (field_type) {
                case "date":
                    ...
                    $date_time_picker
                        ...
                        .then(function () {
                            $date_time_picker.$el.addClass(
                                "ks_dn_filter_first_date_time_widget o_input"
                            );
                            $date_time_picker.setValue(moment());
                        });

                    if (operator.symbol === "between") {
                        var $date_time_picker_2 = new datepicker.DateWidget(
                            this
                        );
                        $date_time_picker_2
                            .appendTo($value_container)
                            .then(function () {
                                $date_time_picker_2.$el.addClass(
                                    "ks_dn_filter_second_date_time_widget o_input"
                                );
                ...
                case "datetime":
                    ...
                    $date_time_picker
                        ...
                        .then(function () {
                            ...
                            $date_time_picker.setValue(
                                moment("00:00:00", "hh:mm:ss")
                            );
                        });

                    if (operator.symbol === "between") {
                        var $date_time_picker_2 = new datepicker.DateTimeWidget(
                            this
                        );
                        $date_time_picker_2
                            .appendTo($value_container)
        ...
        ksOnCustomFilterOperatorSelect: function (ev) {
            ...
            var field_id = $parent_container
                .find(".ks_custom_filter_field_selector")
                .val();
            var field_select =
                this.ks_dashboard_data.ks_dashboard_custom_domain_filter[
                    field_id
        ...
        ksOnCustomFilterApply: function () {
            var model_domain = {};
            $(".ks_dn_custom_filter_input_container_section").each(
                (index, filter_container) => {
                    var field_id = $(filter_container)
                        .find(".ks_custom_filter_field_selector")
                        .val();
                    var field_select =
                        this.ks_dashboard_data
                            .ks_dashboard_custom_domain_filter[field_id];
                    var field_type = field_select.field_type;
                    var domainValue = [];
                    var domainArray = [];
                    var operator =
                    ...
                    if ("value" in operator) {
                        ...
                        var dateValue = [];
                        $(filter_container)
                            .find(".o_generator_menu_value .o_datepicker")
                            .each((index, $input_val) => {
                                var a = $($input_val).find("input").val();
                                var b = field_utils.parse[field_type](
                                    a,
                                    { field_type },
                                    { timezone: true }
                                );
                                var c = field_utils.format[field_type](
                                    b,
                                    { field_type },
                                    { timezone: true }
                                );
                        ...
                        domainValue = [
                            $(filter_container)
                                .find(".o_generator_menu_value input")
                                .val(),
                        ];
                        label =
                            label +
                    ...
                    if (field_select.model in model_domain) {
                        model_domain[field_select.model].domain =
                            model_domain[field_select.model].domain.concat(
                                domainArray
                            );
                        model_domain[field_select.model].domain.unshift("|");
                        model_domain[field_select.model].label =
                            model_domain[field_select.model].label +
                            " or " +
                            label;
                    } else {
        ...
        _ksAddCustomDomain: function (model_domain) {
            ...
            _(model_domain).each((val, model) => {
                ...
                if (domain_data) {
                    ...
                    $(".ks_dn_filter_applied_container").prepend(
                        $filter_container
                    );
                }
        ...
        ksOnRemoveFilterFromSearchPanel: function (ev) {
            ...
            if ($search_section.data("ksCateg")) {
                ...
                var action = "remove_dynamic_filter";
                var $selected_pre_define_filter = $(
                    ".dn_dynamic_filter_selected.dn_filter_click_event_selector[data-ks-categ='" +
                        categ +
                        "']"
                );
                $selected_pre_define_filter.removeClass(
                    "dn_dynamic_filter_selected"
                );
