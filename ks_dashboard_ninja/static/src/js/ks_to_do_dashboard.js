    ...
    return KsDashboard.include({
            ...
            for (var i = 0; i < items.length; i++) {
                if (self.grid) {
                    if (items[i].ks_dashboard_item_type === "ks_tile") {
                        ...
                        var $to_do_preview = self.ksRenderToDoDashboardView(
                            items[i]
                        );
                        if (items[i].id in self.gridstackConfig) {
                            self.grid.addWidget($to_do_preview[0], {
                                x: self.gridstackConfig[items[i].id].x,
                                y: self.gridstackConfig[items[i].id].y,
                                w: self.gridstackConfig[items[i].id].w,
                                h: self.gridstackConfig[items[i].id].h,
        ...
        ksRenderToDoDashboardView: function (item) {
            ...
            var $ks_gridstack_container = $(
                QWeb.render("ks_to_do_dashboard_container", {
        ...
        ksRenderToDoView: function (item, ks_tv_play = false) {
            ...
            var item_id = item.id;
            var list_to_do_data = JSON.parse(item.ks_to_do_data);
            var $todoViewContainer = $(
                QWeb.render("ks_to_do_dashboard_inner_container", {
        ...
        _onKsEditTask: function (e) {
            ...
            var ks_description = $(e.currentTarget.parentElement.parentElement)
                .find(".ks_description")
                .attr("value");

            var $content =
                "<div><input type='text' class='ks_description' value='" +
                ks_description +
                "' placeholder='Task'></input></div>";
                ...
                buttons: [
                    {
                        ...
                        click: function (e) {
                            ...
                            self.onSaveTask(
                                content,
                                parseInt(ks_description_id),
                                parseInt(ks_item_id),
                                parseInt(ks_section_id)
                            );
                        },
                        close: true,
                    },
        ...
        onSaveTask: function (
            ...
            this._rpc({
                ...
                self.ksFetchUpdateItem(ks_item_id).then(function () {
                    $(
                        ".ks_li_tab[data-item-id=" + ks_item_id + "]"
                    ).removeClass("active");
                    $(
                        ".ks_li_tab[data-section-id=" + ks_section_id + "]"
                    ).addClass("active");
                    $(
                        ".ks_tab_section[data-item-id=" + ks_item_id + "]"
                    ).removeClass("active");
                    $(
                        ".ks_tab_section[data-item-id=" + ks_item_id + "]"
                    ).removeClass("show");
                    $(
                        ".ks_tab_section[data-section-id=" + ks_section_id + "]"
                    ).addClass("active");
                    $(
                        ".ks_tab_section[data-section-id=" + ks_section_id + "]"
                    ).addClass("show");
                    $(".header_add_btn[data-item-id=" + ks_item_id + "]").attr(
                        "data-section-id",
                        ks_section_id
                    );
                });
        ...
        _onKsDeleteContent: function (e) {
            ...
            Dialog.confirm(
                this,
                _t("Are you sure you want to remove this task?"),
                {
                    confirm_callback: function () {
                        self._rpc({
                            ...
                            self.ksFetchUpdateItem(ks_item_id).then(
                                function () {
                                    $(
                                        ".ks_li_tab[data-item-id=" +
                                            ks_item_id +
                                            "]"
                                    ).removeClass("active");
                                    $(
                                        ".ks_li_tab[data-section-id=" +
                                            ks_section_id +
                                            "]"
                                    ).addClass("active");
                                    $(
                                        ".ks_tab_section[data-item-id=" +
                                            ks_item_id +
                                            "]"
                                    ).removeClass("active");
                                    $(
                                    ...
                                    $(
                                        ".header_add_btn[data-item-id=" +
                                            ks_item_id +
                                            "]"
                                    ).attr("data-section-id", ks_section_id);
                                }
                            );
                        });
                    },
        ...
        _onKsAddTask: function (e) {
            ...
            var ks_item_id = e.currentTarget.dataset.itemId;
            var $content =
                "<div><input type='text' class='ks_section' placeholder='Task' required></input></div>";
            var dialog = new Dialog(this, {
                title: _t("New Task"),
                $content: $content,
                size: "medium",
        ...
        _onKsUpdateAddButtonAttribute: function (e) {
            ...
            $(".header_add_btn[data-item-id=" + item_id + "]").attr(
                "data-section-id",
                sectionId
            );
        },
