odoo.define("ks_dashboard_ninja.domain_fix", function (require) {
    ...
    BasicModel.include({
        _fetchSpecialDomain: function (record, fieldName, fieldInfo) {
            ...
            if (record._changes && record._changes[fieldName]) {
                if (
                    record._changes[fieldName].includes("%UID") ||
                    record._changes[fieldName].includes("%MYCOMPANY")
                ) {
                    fieldName_temp = fieldName + "_temp";
                    record._changes[fieldName_temp] =
                        record._changes[fieldName];
                    while (record._changes[fieldName_temp].includes("%UID")) {
                        record._changes[fieldName_temp] = record._changes[
                            fieldName_temp
                        ].replace('"%UID"', record.getContext().uid);
                    }
                    while (
                        record._changes[fieldName_temp].includes("%MYCOMPANY")
                    ) {
                        record._changes[fieldName_temp] = record._changes[
                            fieldName_temp
                        ].replace(
                            '"%MYCOMPANY"',
                            this.getSession().user_context
                ...
                (record.data[fieldName].includes("%UID") ||
                    record.data[fieldName].includes("%MYCOMPANY"))
            ) {
                fieldName_temp = fieldName + "_temp";
                record.data[fieldName_temp] = record.data[fieldName];

                while (record.data[fieldName_temp].includes("%UID")) {
                    record.data[fieldName_temp] = record.data[
                        fieldName_temp
                    ].replace('"%UID"', record.getContext().uid);
                }
                while (record.data[fieldName_temp].includes("%MYCOMPANY")) {
                    record.data[fieldName_temp] = record.data[
                        ...
                        '"%MYCOMPANY"',
                        this.getSession().user_context.allowed_company_ids[0]
                    );
                }
            }
    ...
    BasicFields.FieldDomain.include({
        _onShowSelectionButtonClick: function (e) {
            if (
                this.value &&
                (this.value.includes("%MYCOMPANY") ||
                    (this.value && this.value.includes("%UID")))
            ) {
                var temp_value = this.value;
                while (temp_value.includes("%MYCOMPANY")) {
                    var temp_value = temp_value.includes("%MYCOMPANY")
                        ? temp_value.replace(
                              '"%MYCOMPANY"',
                              this.getSession().user_context
                                  .allowed_company_ids[0]
                ...
                while (temp_value.includes("%UID")) {
                    temp_value = temp_value.includes("%UID")
                        ? temp_value.replace(
                              '"%UID"',
                              this.record.getContext().uid
                          )
                        : temp_value;
                }
                e.preventDefault();
