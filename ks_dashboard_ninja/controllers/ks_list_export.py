    ...
    def base(self, data, token):
        ...
        if ks_export_boolean:
            ...
            if ks_start_date and ks_end_date:
                ks_start_date = datetime.datetime.strptime(
                    ks_start_date, DEFAULT_SERVER_DATETIME_FORMAT
                )
                ks_end_date = datetime.datetime.strptime(
                    ks_end_date, DEFAULT_SERVER_DATETIME_FORMAT
                )
            ...
            if item._context.get("ksDateFilterSelection", False):
                ...
                ks_date_filter_selection = (
                    item.ks_dashboard_ninja_board_id.ks_date_filter_selection
                )
                item = item.with_context(
            ...
            if ks_date_filter_selection not in ["l_custom", "l_none"]:
                ks_date_data = ks_get_date(
                    ks_date_filter_selection, request, "datetime"
                )
                item = item.with_context(
            ...
            ks_chart_domain = item.ks_convert_into_proper_domain(
                item.ks_domain, item, item_domain
            )
            # list_data = item.ks_fetch_list_view_data(item,ks_chart_domain, ks_export_all=
            if list_data["type"] == "ungrouped":
                list_data = item.ks_fetch_list_view_data(
                    item, ks_chart_domain, ks_export_all=True
                )
            elif list_data["type"] == "grouped":
                list_data = item.get_list_view_record(
                    orderby, sort_order, ks_chart_domain, ks_export_all=True
                )
                ...
                if isinstance(val, (float, int)):
                    if val >= 0:
                        ...
                        dataset["data"][ks_count] = (
                            item.env["ir.qweb.field.float"]
                            .sudo()
                            .value_to_html(val, {"precision": ks_precision})
                        )
