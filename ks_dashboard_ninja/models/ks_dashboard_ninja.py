...
class KsDashboardNinjaBoard(models.Model):
    ...
    name = fields.Char(string="Dashboard Name", required=True, size=35, translate=1)
    ks_dashboard_items_ids = fields.One2many(
        "ks_dashboard_ninja.item",
        "ks_dashboard_ninja_board_id",
        string="Dashboard Items",
    )
    ks_dashboard_menu_name = fields.Char(string="Menu Name", translate=1)
    ...
    ks_data_formatting = fields.Selection(
        [("global", "Global"), ("indian", "Indian"), ("exact", "Exact")],
        string="Format",
    )
    ...
    def create(self, vals):
        ...
        if "ks_dashboard_top_menu_id" in vals and "ks_dashboard_menu_name" in vals:
            ...
            record.ks_dashboard_client_action_id = (
                ...
                .create(
                    {
                        ...
                        + str(record.ks_dashboard_client_action_id.id),
        ...
        if (
            record.ks_dashboard_default_template
            and record.ks_dashboard_default_template.ks_item_count
        ):
            ks_gridstack_config = {}
            template_data = json.loads(
                record.ks_dashboard_default_template.ks_gridstack_config
            )
    ...
    def write(self, vals):
        if (
            vals.get("ks_date_filter_selection", False)
            and vals.get("ks_date_filter_selection") != "l_custom"
        ):
            vals.update(
        ...
        for rec in self:
            if "ks_dashboard_menu_name" in vals:
                if (
                    ...
                    and self.env.ref("ks_dashboard_ninja.ks_my_default_dashboard_board")
                    .sudo()
                    .id
                    == rec.id
                ):
                    if self.env.ref("ks_dashboard_ninja.board_menu_root", False):
                        ...
                        ).sudo().name = vals["ks_dashboard_menu_name"]
                else:
                    rec.ks_dashboard_menu_id.sudo().name = vals[
                        "ks_dashboard_menu_name"
                    ]
            if "ks_dashboard_group_access" in vals:
                if (
                    self.env.ref("ks_dashboard_ninja.ks_my_default_dashboard_board").id
                    == rec.id
                ):
                    if self.env.ref("ks_dashboard_ninja.board_menu_root", False):
                        self.env.ref(
                    ...
                    rec.ks_dashboard_menu_id.sudo().groups_id = vals[
                        "ks_dashboard_group_access"
                    ]
            ...
            if "ks_dashboard_menu_sequence" in vals:
                rec.ks_dashboard_menu_id.sudo().sequence = vals[
    ...
    def ks_fetch_dashboard_data(self, ks_dashboard_id, ks_item_domain=False):
        ...
        has_group_ks_dashboard_manager = self.env.user.has_group(
            "ks_dashboard_ninja.ks_dashboard_ninja_group_manager"
        )
        ...
        dashboard_data = {
            ...
            "ks_date_filter_selection": self._context.get(
            ...
            "ks_item_model_relation": {
                ...
                for x in ks_dashboard_rec.ks_dashboard_items_ids.read(
        ...
        if self.env["ks_dashboard_ninja.child_board"].search(
            ...
            dashboard_data["ks_child_boards"] = {
                "ks_default": [
                    ks_dashboard_rec.name,
                    default_grid_id.ks_gridstack_config,
                ]
            }
            ...
            for rec in self.env["ks_dashboard_ninja.child_board"].search_read(
                ...
                dashboard_data["ks_child_boards"][str(rec["id"])] = [
                    rec["name"],
                    rec["ks_gridstack_config"],
                ]
    ...
    def ks_fetch_item_data(self, rec, params={}):
        ...
        """
        try:
            ks_precision = self.sudo().env.ref(
        ...
        item = {
            ...
            "ks_domain": rec.ks_convert_into_proper_domain(
                rec.ks_domain, rec, item_domain1
            ),
            ...
            "ks_list_view_group_fields": rec.ks_list_view_group_fields.ids
            if rec.ks_list_view_group_fields
            else False,
    ...
    def ks_set_date(self, ks_dashboard_id):
        ...
        if self._context.get("ksDateFilterSelection", False):
            ...
            if ks_date_filter_selection == "l_custom":
                ...
                self = self.with_context(
                    ksDateFilterStartDate=fields.datetime.strptime(
                        ks_start_dt_parse.strftime("%Y-%m-%d %H:%M:%S"),
                        "%Y-%m-%d %H:%M:%S",
                    )
                )
                self = self.with_context(
                    ksDateFilterEndDate=fields.datetime.strptime(
                        ks_end_dt_parse.strftime("%Y-%m-%d %H:%M:%S"),
                        "%Y-%m-%d %H:%M:%S",
                    )
                )
            ...
            self = self.with_context(
                ksDateFilterStartDate=ks_dashboard_rec.ks_dashboard_start_date
            )
            self = self.with_context(
                ksDateFilterEndDate=ks_dashboard_rec.ks_dashboard_end_date
            )
    ...
    @api.model
    def ks_get_list_view_data_offset(
        self, ks_dashboard_item_id, offset, dashboard_id, params={}
    ):
    ...
    def ks_export_item(self, item_id):
        return {
            ...
            "item": self.ks_export_item_data(
                self.ks_dashboard_items_ids.browse(int(item_id))
            ),
        }
    ...
    def ks_export_item_data(self, rec):
        ...
        for res in rec.ks_goal_lines:
            goal_line = {
                "ks_goal_date": datetime.datetime.strftime(
                    res.ks_goal_date, "%Y-%m-%d"
                ),
        ...
        selecred_rec = self.env["ks_dashboard_ninja.child_board"].search(
            [
                [
                    "id",
                    "in",
                    rec.ks_dashboard_ninja_board_id.ks_child_dashboard_ids.ids,
                ],
                ["ks_active", "=", True],
                ["company_id", "=", self.env.company.id],
            ],
        ...
        rec.ks_dashboard_ninja_board_id.ks_get_grid_config()
        ...
        elif rec.ks_dashboard_ninja_board_id.ks_child_dashboard_ids[
            0
        ].ks_gridstack_config:
            keys_data = json.loads(
                rec.ks_dashboard_ninja_board_id.ks_child_dashboard_ids[
                    0
                ].ks_gridstack_config
            )
        else:
        ...
        item = {
            ...
            "ks_client_action": rec.ks_client_action.xml_id
            if rec.ks_client_action
            else False,
            "ks_is_client_action": rec.ks_is_client_action,
            "ks_export_all_records": rec.ks_export_all_records,
            ...
            "ks_multiplier_active": rec.ks_multiplier_active,
            "ks_multiplier": rec.ks_multiplier,
            "ks_multiplier_lines": ks_multiplier_lines
            if ks_multiplier_lines
            else False,
        }
        if grid_corners:
            item.update(
    ...
    def ks_open_import(self, **kwargs):
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "ks_dashboard_ninja.ks_import_dashboard_action"
        )
        return action

    def ks_open_setting(self, **kwargs):
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "ks_dashboard_ninja.board_form_tree_action_window"
        )
    ...
    def ks_import_item(self, dashboard_id, **kwargs):
        ...
        if (
            ...
            == "ks_dashboard_ninja_item_export"
        ):
    ...
    def ks_dashboard_export(self, ks_dashboard_ids, **kwargs):
        ...
        for ks_dashboard_id in ks_dashboard_ids:
            ...
            if selecred_rec:
                ...
                dashboard_data[
                    "ks_gridstack_config"
                ] = ks_dashboard_rec.ks_child_dashboard_ids.ks_gridstack_config
    ...
    def ks_prepare_item(self, item):
        ...
        if item["ks_date_filter_field"]:
            ...
            ks_record_id = self.env["ir.model.fields"].search(
                [
                    ("name", "=", ks_date_filter_field),
                    ("model", "=", item["ks_model_id"]),
                ]
            )
            if ks_record_id:
        ...
        if item["ks_list_target_deviation_field"]:
            ...
            record_id = self.env["ir.model.fields"].search(
                [
                    ("name", "=", ks_list_target_deviation_field),
                    ("model", "=", item["ks_model_id"]),
                ]
            )
            if record_id:
        ...
        ks_model_id = (
            self.env["ir.model"].search([("model", "=", item["ks_model_id"])]).id
        )
        ...
        if item["ks_model_id_2"]:
            ...
            ks_model_id_2 = (
                self.env["ir.model"].search([("model", "=", item["ks_model_id_2"])]).id
            )
            if item["ks_record_field_2"]:
                ks_record_field = item["ks_record_field_2"]
                ks_record_id = self.env["ir.model.fields"].search(
                    [
                        ("model", "=", item["ks_model_id_2"]),
                        ("name", "=", ks_record_field),
                    ]
                )
            ...
            if item["ks_date_filter_field_2"]:
                ks_record_id = self.env["ir.model.fields"].search(
                    [
                        ("model", "=", item["ks_model_id_2"]),
                        ("name", "=", item["ks_date_filter_field_2"]),
                    ]
                )
        ...
        item["ks_item_start_date"] = (
            item["ks_item_start_date"] if item["ks_item_start_date"] else False
        )
        item["ks_item_end_date"] = (
            item["ks_item_end_date"] if item["ks_item_end_date"] else False
        )
        item["ks_item_start_date_2"] = (
            item["ks_item_start_date_2"] if item["ks_item_start_date_2"] else False
        )
        item["ks_item_end_date_2"] = (
            item["ks_item_end_date_2"] if item["ks_item_end_date_2"] else False
        )
    ...
    def update_child_board(self, action, dashboard_id, data):
        ...
        if action == "create":
            ...
            if data["ks_selected_board_id"] != "ks_default":
                ...
                result = dashboard_id.ks_child_dashboard_ids.browse(
                    int(data["ks_selected_board_id"])
                ).write({"ks_active": True})
            else:
                result = dashboard_id.ks_child_dashboard_ids.search(
                    [["ks_active", "=", True]]
                ).write({"ks_active": False})
    ...
    def ks_prepare_dashboard_domain(self):
        pre_defined_filter_ids = self.env[
            "ks_dashboard_ninja.board_defined_filters"
        ].search(
            [
        ...
        for model_id in filter_model_ids:
            ...
            for rec in filter_ids:
                ...
                if data.get(rec.ks_model_id.model) and rec.ks_domain:
                    data[rec.ks_model_id.model]["domain"] = data[rec.ks_model_id.model][
                        "domain"
                    ] + safe_eval(rec.ks_domain)
    ...
    def ks_prepare_dashboard_pre_domain(self):
        ...
        for rec in pre_defined_filter_ids:
            ...
            data[rec["id"]] = {
                ...
                "categ": rec.ks_model_id.model + "_" + str(categ_seq)
                if rec.display_type != "line_section"
                else 0,
