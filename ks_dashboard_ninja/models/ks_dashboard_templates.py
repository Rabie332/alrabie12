...
class KsDashboardNinjaTemplate(models.Model):
    ...
    ks_template_type = fields.Selection(
        [("ks_default", "Predefined"), ("ks_custom", "Custom")],
        string="Template Format",
    )
    ks_dashboard_item_ids = fields.One2many(
        "ks_dashboard_ninja.item",
        "ks_dashboard_board_template_id",
    ...
    def create(self, vals_list):
        for val in vals_list:
            if val.get("ks_template_type", False) and val.get(
                "ks_dashboard_board_id", False
            ):
                dashboard_id = self.env["ks_dashboard_ninja.board"].browse(
                    val.get("ks_dashboard_board_id")
                )
                ...
                val["ks_dashboard_item_ids"] = [
                    (4, x.copy({"ks_dashboard_ninja_board_id": False}).id)
                    for x in dashboard_id.ks_dashboard_items_ids
                ]
        ...
        if val.get("ks_dashboard_board_id", False):
            ...
            val["ks_dashboard_item_ids"] = [
                (
                    ...
                    0,
                    [
                        x.copy({"ks_dashboard_ninja_board_id": False}).id
                        for x in dashboard_id.ks_dashboard_items_ids
                    ],
                )
            ]
