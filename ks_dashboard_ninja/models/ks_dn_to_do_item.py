...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_to_do_data = fields.Char(
        string="To Do Data in JSon",
        compute="ks_get_to_do_view_data",
        compute_sudo=False,
    )
    ...
    def _ksGetToDOData(self):
        ...
        if self.ks_dn_header_lines:
            for ks_dn_header_line in self.ks_dn_header_lines:
                ...
                if (
                    type(ks_dn_header_line.id).__name__ != "int"
                    and ks_dn_header_line.id.ref != None
                ):
                ...
                if " " in ks_dn_header_line.ks_to_do_header:
                    ...
                    ks_to_do_data["ks_link"].append(
                        "#" + ks_temp + ks_dn_header_line_id
                    )
                    ks_to_do_data["ks_href_id"].append(
                        ks_temp + str(ks_dn_header_line.id)
                    )
                    ...
                    ks_to_do_data["ks_link"].append(
                        "#" + ks_dn_header_line.ks_to_do_header + ks_dn_header_line_id
                    )
                    ks_to_do_data["ks_href_id"].append(
                ...
                if len(ks_dn_header_line.ks_to_do_description_lines):
                    for (
                        ks_to_do_description_line
                    ) in ks_dn_header_line.ks_to_do_description_lines:
                        if (
                            " " in ks_dn_header_line.ks_to_do_header
                            or ks_dn_header_line.ks_to_do_header[0].isdigit()
                        ):
                            if ks_to_do_data["ks_content"].get(
                                ks_temp + str(ks_dn_header_line.id), False
                            ):
                            ...
                            if ks_to_do_data["ks_content"].get(
                                ks_dn_header_line.ks_to_do_header
...
class KsToDoheaders(models.Model):
    ...
    ks_to_do_description_lines = fields.One2many(
        "ks_to.do.description", "ks_to_do_header_id"
    )
            ...
            if rec.ks_to_do_header:
                ...
                if not ks_check:
                    raise ValidationError(
                        _(
                            "Special characters are not allowed only string and digits allow for section header"
                        )
                    )
