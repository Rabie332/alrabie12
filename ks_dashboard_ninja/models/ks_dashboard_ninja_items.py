...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_dashboard_board_template_id = fields.Many2one(
        "ks_dashboard_ninja.board_template", string="Dashboard Template"
    )
    ...
    ks_model_name_2 = fields.Char(
        related="ks_model_id_2.model", string="Kpi Model Name"
    )
    ...
    ks_default_icon = fields.Char(
        string="Icon", default="bar-chart", help="Select the icon to be displayed. "
    )
    ...
    ks_icon_select = fields.Char(
        string="Icon Option", default="Default", help="Choose the Icon option. "
    )
    ks_font_color = fields.Char(
        default="#ffffff,0.99", string="Font Color", help="Select the font color. "
    )
    ...
    ks_record_data_limit_visibility = fields.Boolean(
        string="Record Limit Data Visibility",
    ...
    ks_dashboard_ninja_board_id = fields.Many2one(
        ...
        if "ks_dashboard_id" in self._context
        else False,
    )
    ...
    ks_chart_groupby_type = fields.Char(
        compute="get_chart_groupby_type", compute_sudo=False
    )
    ks_chart_sub_groupby_type = fields.Char(
        compute="get_chart_sub_groupby_type", compute_sudo=False
    )
    ...
    ks_chart_data = fields.Char(
        string="Chart Data in string form",
        compute="ks_get_chart_data",
        compute_sudo=False,
    )
    ...
    ks_bar_chart_stacked = fields.Boolean(
        string="Stacked Bar Chart", help="Stack the columns of the same record. "
    )
    ...
    ks_sort_by_order = fields.Selection(
        ...
        string="Sort Order",
        help=" Select the order of the sorting. ",
    )
    ks_record_data_limit = fields.Integer(
        string="Record Limit", help=" Records to be displayed on the graph"
    )

    ks_list_view_preview = fields.Char(
        string="List View Preview", default="List View Preview"
    )
    ...
    ks_target_view = fields.Char(
        string="View",
        default="Number",
        help=" Select the view to compare target with data.",
    )

    ks_data_comparison = fields.Char(string="Kpi Data Type", default="None")

    ks_kpi_data = fields.Char(
        string="KPI Data", compute="ks_get_kpi_data", compute_sudo=False
    )
    ...
    ks_standard_goal_value = fields.Float(
        string="Standard Target", help="Show the set target"
    )
    ks_goal_lines = fields.One2many(
        "ks_dashboard_ninja.item_goal", "ks_dashboard_item", string="Target Lines"
    )
    ...
    ks_show_data_value = fields.Boolean(
        string="Show Data Value", help=" Display value on the graph. . "
    )

    ks_action_lines = fields.One2many(
        "ks_dashboard_ninja.item_action", "ks_dashboard_item_id", string="Action Lines"
    )
    ...
    ks_year_period = fields.Integer(
        string="Same Period Previous Years",
    ...
    ks_multiplier_active = fields.Boolean(
        string="Apply Multiplier",
        default=False,
        help="Provides the visibility of multiplier field",
    )
    ks_multiplier = fields.Float(
        string="Multiplier",
        default=1,
        help="Provides the multiplication of record value",
    )
    ...
    ks_update_items_data = fields.Selection(
        ...
        default=lambda self: self._context.get("ks_set_interval", False),
        help=" Data will be refreshed after the selected interval.",
    )

    # User can select custom units for measure
    ks_unit = fields.Boolean(
        string="Show Custom Unit", default=False, help="Display the unit of the data."
    )
    ...
    ks_domain_extension = fields.Char(
        "Domain Extension", help="Define conditions for filter to write manually"
    )
    ks_domain_extension_2 = fields.Char("KPI Domain Extension")
    # hide legend
    ks_hide_legend = fields.Boolean(
        "Show Legend", help="Hide all legend from the chart item", default=True
    )
    ...
    ks_auto_update_type = fields.Selection(
        ...
        default=lambda self: "ks_update_interval"
        if self._context.get("ks_set_interval", False)
        else False,
        help="Select the update type.",
    )
    ks_show_live_pop_up = fields.Boolean(
        string="Show Live Update Pop Up",
    ...
    ks_precision_digits = fields.Integer(
        "Digits", compute="_ks_compute_precision_digits", store=True, readonly=False
    )
    ...
    def _ks_compute_precision_digits(self):
        for rec in self:
            try:
                precision_digits = self.sudo().env.ref(
        ...
        for rec in self:
            ...
            if rec.ks_multiplier_active:
                if (
                    ...
                    and rec.ks_list_view_type == "grouped"
                ):
                ...
                if rec.ks_chart_measure_field_2:
                    for ks_chart_measure_field in rec.ks_chart_measure_field_2:
                        if ks_chart_measure_field.ids[0] not in ks_chart_measure_id:
                            ...
                            ks_line = self.env["ks_dashboard_item.multiplier"].create(
                                ks_dict
                            )
                            ks_temp_list.append(ks_line.id)
        ...
        for rec in self:
            if rec.ks_pagination_limit > 0:
                ...
                raise ValidationError(
                    _("Pagination limit value cannot be Negative or Zero")
                )
            if (
                rec.ks_goal_lines
                and rec.ks_pagination_limit > 0
                or rec.ks_pagination_limit < 0
            ):
                raise ValidationError(
                    _("if target lines is selected then cannot be set pagination value")
                )
    ...
    def ks_date_target_line(self):
        for rec in self:
            if rec.ks_chart_date_groupby in (
                "minute",
                "hour",
            ) or rec.ks_chart_date_sub_groupby in ("minute", "hour"):
                rec.ks_goal_lines = False
                return {
                    "warning": {
    ...
    def ks_date_target(self):
        for rec in self:
            if (
                rec.ks_chart_date_groupby in ("minute", "hour")
                or rec.ks_chart_date_sub_groupby in ("minute", "hour")
            ...
            if (
                ...
                and rec.ks_chart_date_groupby in ("minute", "hour")
            ):
                raise ValidationError(
                    _("Groupby field: {} cannot be aggregated by {}").format(
                        rec.ks_chart_relation_groupby.display_name,
                        rec.ks_chart_date_groupby,
                    )
                )
            if (
                rec.ks_chart_relation_sub_groupby.ttype == "date"
                and rec.ks_chart_date_sub_groupby in ("minute", "hour")
            ):
                raise ValidationError(
                    _("Groupby field: {} cannot be aggregated by {}").format(
                        rec.ks_chart_relation_sub_groupby.display_name,
    ...
    def copy_data(self, default=None):
        ...
        if "ks_action_lines" not in default:
            default["ks_action_lines"] = [
                (0, 0, line.copy_data()[0]) for line in self.ks_action_lines
            ]

        if "ks_goal_lines" not in default:
            default["ks_goal_lines"] = [
                (0, 0, line.copy_data()[0]) for line in self.ks_goal_lines
            ]
    ...
    def create(self, values):
        ...
        if values.get("ks_list_view_fields", False) and values.get(
            "ks_list_view_group_fields", False
        ):
            ...
            values["ks_many2many_field_ordering"] = json.dumps(
                ks_many2many_field_ordering
            )
    ...
    def write(self, values):
        for rec in self:
            if rec["ks_many2many_field_ordering"]:
                ks_many2many_field_ordering = json.loads(
                    rec["ks_many2many_field_ordering"]
                )
            else:
                ks_many2many_field_ordering = {}
            if values.get("ks_list_view_fields", False):
                ks_many2many_field_ordering["ks_list_view_fields"] = values[
                    "ks_list_view_fields"
                ][0][2]
            if values.get("ks_list_view_group_fields", False):
                ks_many2many_field_ordering["ks_list_view_group_fields"] = values[
                    "ks_list_view_group_fields"
                ][0][2]
            values["ks_many2many_field_ordering"] = json.dumps(
                ks_many2many_field_ordering
        ...
        if self.ks_dashboard_item_theme != "white":
            if self.ks_layout == "layout4":
                ...
                self.ks_default_icon_color = self.ks_get_dark_color(
                    self.ks_background_color.split(",")[0],
    ...
    def _ksGetRecordCount(self, domain=[]):
        rec = self
        if (
            rec.ks_record_count_type == "count"
            or rec.ks_dashboard_item_type == "ks_list_view"
        ):
            ks_record_count = rec.ks_fetch_model_data(
                rec.ks_model_name, rec.ks_domain, "search_count", rec, domain
            )
        elif (
    ...
    def ks_fetch_model_data(self, ks_model_name, ks_domain, ks_func, rec, domain=[]):
        ...
        try:
            if ks_domain and ks_domain != "[]" and ks_model_name:
                proper_domain = self.ks_convert_into_proper_domain(
    ...
    def ks_convert_into_proper_domain(self, ks_domain, rec, domain=[]):
        ...
        if rec.ks_date_filter_field:
            if (
                not rec.ks_date_filter_selection
                or rec.ks_date_filter_selection == "l_none"
            ):
                ...
                ks_is_def_custom_filter = self._context.get(
                ...
                if (
                    ...
                    selected_start_date = ks_convert_into_local(
                        selected_start_date, ks_timezone
                    )
                ...
                if self._context.get("ksDateFilterSelection", False) and self._context[
                    "ksDateFilterSelection"
                ] not in ["l_none", "l_custom"]:
                    ks_date_data = ks_get_date(
                        self._context.get("ksDateFilterSelection"),
                        self,
                ...
                if (
                    rec.ks_date_filter_selection
                    and rec.ks_date_filter_selection != "l_custom"
                ):
                    ks_date_data = ks_get_date(
                        rec.ks_date_filter_selection,
                        self,
                    ...
                    if rec.ks_item_start_date or rec.ks_item_end_date:
                        ...
                        if (
                            rec.ks_date_filter_field.ttype == "date"
                            and rec.ks_item_start_date
                            and rec.ks_item_end_date
                        ):
                            ks_timezone = self._context.get("tz") or self.env.user.tz
                            selected_start_date = ks_convert_into_local(
                                rec.ks_item_start_date, ks_timezone
                            )
                            selected_end_date = ks_convert_into_local(
                                rec.ks_item_end_date, ks_timezone
                            )
                ...
                if selected_start_date and selected_end_date:
                    if rec.ks_compare_period:
                        ...
                        if rec.ks_compare_period > 0:
                            ...
                            if (
                                rec.ks_date_filter_field.ttype == "date"
                                and rec.ks_date_filter_selection == "l_day"
                            ):
                                selected_end_date = selected_end_date + timedelta(
                                    days=ks_compare_period
                                )
                            ...
                            if (
                                ...
                                selected_start_date = selected_end_date - timedelta(
                                    days=ks_compare_period
                                )

                    if (
                        rec.ks_year_period
                        and rec.ks_year_period != 0
        ...
        if rec.ks_domain_extension:
            ks_domain_extension = rec.ks_convert_domain_extension(
                rec.ks_domain_extension, rec
            )
    ...
    def ks_convert_domain_extension(self, ks_extensiom_domain, rec):
        if ks_extensiom_domain and "%UID" in ks_extensiom_domain:
            ks_extensiom_domain = ks_extensiom_domain.replace(
        ...
        if self.ks_domain_extension:
            try:
                ...
                if "%UID" in ks_domain_extension:
                    ks_domain_extension = ks_domain_extension.replace(
                        "%UID", str(self.env.user.id)
                    )
                ...
                self.env[self.ks_model_name].search_count(
                    safe_eval(ks_domain_extension)
                )
            except Exception:
                raise ValidationError(
    ...
    def get_chart_groupby_type(self):
        for rec in self:
            if (
                rec.ks_chart_relation_groupby.ttype == "datetime"
                or rec.ks_chart_relation_groupby.ttype == "date"
            ):
    ...
    def ks_empty_limit(self):
        for rec in self:
            if (
                rec.ks_chart_relation_sub_groupby
                or rec.ks_fill_temporal
                or rec.ks_goal_lines
            ):
                rec.ks_record_data_limit = 0
    ...
    def _ks_get_chart_data(self, domain=[]):
        ...
        if (
            ...
            if rec.ks_chart_data_count_type == "count":
                ...
                if rec.ks_dashboard_item_type == "ks_bar_chart":
                    ...
                    for res in rec.ks_chart_measure_field_2:
                        ...
                        ks_chart_measure_field_with_type_2.append(
                            res.name + ":" + ks_data_count_type
                        )
                ...
                for res in range(0, len(rec.ks_chart_measure_field)):
                    ...
                    ks_chart_measure_field_ids.append(
                        rec.ks_chart_measure_field[res].ids[0]
                    )

                    if len(rec.ks_chart_cumulative_field) > len(
                    ...
                    if (
                        ...
                        and (
                            rec.ks_chart_cumulative_field[res].id
                            or rec.ks_chart_cumulative_field[res].id.origin
                        )
                        ...
                        ks_chart_data["datasets"].append(
                            {
                                "data": [],
                                "label": rec.ks_chart_cumulative_field[
                                    res
                                ].field_description,
                        ...
                        ks_chart_data["datasets"].append(
                            {
                                "data": [],
                                "label": rec.ks_chart_measure_field[
                                    res
                                ].field_description,
            ...
            ks_chart_domain = self.ks_convert_into_proper_domain(
                rec.ks_domain, rec, domain
            )
            ks_chart_data["previous_domain"] = ks_chart_domain
            if (
                rec.ks_chart_data_count_type == "count"
                and not self.ks_fill_temporal
                and not rec.ks_sort_by_field
            ):
                orderby = "count"
            else:
            ...
            limit = (
                rec.ks_record_data_limit
                if rec.ks_record_data_limit and rec.ks_record_data_limit > 0
                else 5000
            )
            ...
            if (
                ...
                or (
                    rec.ks_chart_data_count_type == "count"
                    and not ks_chart_measure_field
                )
                ...
                if (
                    ...
                    if (
                        ...
                        if rec.ks_date_filter_selection == "l_none":
                            selected_start_date = rec._context.get(
                                "ksDateFilterStartDate", False
                            )
                            selected_end_date = rec._context.get(
                            ...
                            if rec.ks_date_filter_selection == "l_custom":
                                ...
                                selected_start_date = ks_date_data[
                                    "selected_start_date"
                                ]
                                selected_end_date = ks_date_data["selected_end_date"]
                        ...
                        if rec.ks_chart_date_groupby == "month_year":
                            ...
                            ks_chart_date_groupby = rec.ks_chart_date_groupby
                        if (
                            ks_date_data["start_date"]
                            and ks_date_data["end_date"]
                            and rec.ks_goal_lines
                        ):
                        ...
                        ks_goal_records = self.env[
                            "ks_dashboard_ninja.item_goal"
                        ].read_group(
                            ks_goal_domain,
                            ["ks_goal_value"],
                        ...
                        if rec.ks_goal_lines and len(rec.ks_goal_lines) != 0:
                            ...
                            for label in ks_chart_records:
                                ...
                                if domain:
                                    ...
                                    ks_chart_data["domains"].append(
                                        ks_goal_domains.get(label, [])
                                    )
                                counterr = 0
                ...
                if rec.ks_chart_relation_sub_groupby.ttype == "date":
                    ...
                    if rec.ks_chart_date_groupby == "month_year":
                        ...
                        ks_chart_date_groupby = rec.ks_chart_date_groupby
                if (
                    len(ks_chart_measure_field) != 0
                    or rec.ks_chart_data_count_type == "count"
                ):
                    if (
                        rec.ks_chart_groupby_type == "date_type"
                        and ks_chart_date_groupby
                    ):
                        ks_chart_group = (
                        ...
                        if res.get(ks_chart_groupby_relation_fields[0], False):
                            if rec.ks_chart_groupby_type == "date_type":
                                ...
                                if (
                                    ...
                                    label = " ".join(
                                        res[ks_chart_groupby_relation_fields[0]].split(
                                            " "
                                        )[0:2]
                                    )
                                    ...
                                    "minute",
                                    "hour",
                                ] and rec.ks_chart_date_sub_groupby in [
                                    "month",
                                    "week",
                                    "quarter",
                                    "year",
                                    ...
                                    label = res[
                                        ks_chart_groupby_relation_fields[0]
                                    ].split(" ")[0]
                            ...
                            if rec.ks_chart_data_count_type != "count":
                                ...
                                if (
                                    ...
                                    and rec.ks_dashboard_item_type == "ks_bar_chart"
                                ):
                ...
                for ks_multiplier in self.ks_multiplier_lines:
                    for i in range(0, len(ks_chart_data["datasets"])):
                        if (
                            ...
                            data_values = list(
                                map(
                                    lambda x: ks_multiplier.ks_multiplier_value * x,
                                    data_values,
                                )
                            )
    ...
    def _ksGetListViewData(self, domain=[]):
        ...
        if (
            ...
            ks_list_view_data = rec.get_list_view_record(
                orderby, sort_order, ks_chart_domain
            )
    ...
    def get_list_view_record(
        ...
        ks_limit = (
            self.ks_record_data_limit
            if self.ks_record_data_limit and self.ks_record_data_limit > 0
            else False
        )
        ...
        if self.ks_list_view_type == "ungrouped":
            ...
            self.ks_list_view_type == "grouped"
            and self.ks_list_view_group_fields
            ...
            if self.ks_chart_groupby_type == "relational_type":
                ...
                ks_list_view_data["fields"].append(
                    self.ks_chart_relation_groupby.ids[0]
                )
                ks_list_view_data["fields_type"].append(
                    self.ks_chart_relation_groupby.ttype
                )
                ...
                for res in ks_list_view_records:
                    if (
                        ...
                        for field_rec in ks_list_fields:
                            ...
                            counter += 1
                            data_row["ks_column_type"].append(
                                self.ks_chart_relation_groupby.ttype
                            )
                        ks_list_view_data["data_rows"].append(data_row)

            elif (
                self.ks_chart_groupby_type == "date_type" and self.ks_chart_date_groupby
            ):
                ...
                ks_list_view_data["label"].append(
                    self.ks_chart_relation_groupby.field_description
                    + " : "
                    + ks_chart_date_groupby.capitalize()
                )
                ks_list_view_data["fields"].append(
                    self.ks_chart_relation_groupby.ids[0]
                )
                ...
                if self.ks_goal_enable and self.ks_list_target_deviation_field:
                    list_target_deviation_field.append(
                        self.ks_list_target_deviation_field.name
                    )
                    if self.ks_list_target_deviation_field.name in ks_list_field:
                        ...
                        ks_list_view_data["label"].remove(
                    ...
                    if all(list_fields in res for list_fields in ks_list_fields):
                        ...
                        if res[ks_selection_field]:
                            data_row["data"].append(
                                dict(
                                    self.env[self.ks_model_name].fields_get(
                                        allfields=ks_selection_field
                                    )[ks_selection_field]["selection"]
            ...
            for ks_multiplier in self.ks_multiplier_lines:
                ...
                if label in ks_list_view_data["label"]:
                    ...
                    for i in range(0, len(ks_list_view_data["data_rows"])):
                        data_values = (
                            ks_list_view_data["data_rows"][i]["data"][index]
                            * ks_multiplier.ks_multiplier_value
                        )
    ...
    def get_target_list_view_data(
        ...
        for res in ks_list_view_records:
            ...
            ks_list_records[res[ks_group_by]] = {
                "measure_field": [],
                "deviation_value": 0.0,
            }
        ...
        if rec.ks_goal_lines and len(rec.ks_goal_lines) != 0:
            ...
            for res in ks_goal_records:
                if res["ks_goal_date" + ":" + ks_chart_date_groupby]:
                    ks_goal_labels.append(
                        res["ks_goal_date" + ":" + ks_chart_date_groupby]
                    )
                    ks_goal_dataset[
                        res["ks_goal_date" + ":" + ks_chart_date_groupby]
                    ] = res["ks_goal_value"]
    ...
    def ks_fetch_list_view_data(
        ...
        for res in ks_list_view_records:
            ...
            for field_rec in ks_list_view_fields:
                ...
                elif ks_list_view_field_type[counter] == "selection" and res.get(
                    field_rec, False
                ):
                    res[field_rec] = dict(
                        self.env[rec.ks_model_name].fields_get(allfields=[field_rec])[
                            field_rec
                        ]["selection"]
                    )[res[field_rec]]
                data_row["data"].append(res[field_rec])
    ...
    def set_color_palette(self):
        for rec in self:
            ...
            if (
                ...
                or rec.ks_dashboard_item_type == "ks_tile"
            ):
    ...
    def _ksGetKpiData(self, domain1=[], domain2=[]):
        ...
        if (
            rec.ks_dashboard_item_type
            and rec.ks_dashboard_item_type == "ks_kpi"
            and rec.ks_model_id
        ):
            ks_kpi_data = []
            ks_record_count = 0.0
            ...
            if rec.ks_model_id_2 and rec.ks_record_count_type_2:
                ...
                ks_kpi_data_model_2["record_field"] = (
                    ...
                    else rec.ks_record_field_2.field_description
                )
    ...
    def ks_get_previous_period_data(self, rec):
        ...
        if rec.ks_date_filter_selection == "l_none":
            date_filter_selection = (
                rec.ks_dashboard_ninja_board_id.ks_date_filter_selection
            )
        else:
        ...
        if ks_previous_period:
            ks_date_data = ks_get_date(
                ks_previous_period, self, rec.ks_date_filter_field.ttype
            )
        ...
        if ks_date_data:
            ...
            if rec.ks_record_count_type == "count":
                ks_record_count = 0
                try:
                    ks_record_count = self.env[rec.ks_model_name].search_count(
                        proper_domain
                    )
                except Exception:
                    ks_record_count = 0
                ...
                if rec.ks_record_count_type == "sum":
                    return (
                        ...
                        and (data.get(rec.ks_record_field.name))
                        else 0
                    )
                else:
                    return (
    ...
    def ks_get_previous_period_domain(
        self, ks_domain, ks_start_date, ks_end_date, date_filter_field
    ):
        if ks_domain and "%UID" in ks_domain:
            ks_domain = ks_domain.replace('"%UID"', str(self.env.user.id))
        if ks_domain:
    ...
    def ks_fetch_model_data_2(self, ks_model_name, ks_domain, ks_func, rec, domain=[]):
        ...
        try:
            if ks_domain and ks_domain != "[]" and ks_model_name:
                proper_domain = self.ks_convert_into_proper_domain_2(
                    ks_domain, rec, domain
                )
                if ks_func == "search_count":
                    data = self.env[ks_model_name].search_count(proper_domain)
    ...
    def ks_set_date_filter_2(self):
        for rec in self:
            if (
                not rec.ks_date_filter_selection_2
            ) or rec.ks_date_filter_selection_2 == "l_none":
                rec.ks_item_start_date_2 = rec.ks_item_end_date = False
            elif rec.ks_date_filter_selection_2 != "l_custom":
                ks_date_data = ks_get_date(
                    rec.ks_date_filter_selection_2,
                    self,
    ...
    def ks_convert_into_proper_domain_2(self, ks_domain_2, rec, domain=[]):
        ...
        if rec.ks_date_filter_field_2:
            if (
                ...
                if (
                    rec.ks_date_filter_selection_2
                    and rec.ks_date_filter_selection_2 != "l_custom"
                ):
                    ks_date_data = ks_get_date(
                        rec.ks_date_filter_selection_2,
                        self,
                    ...
                    if rec.ks_item_start_date_2 or rec.ks_item_end_date_2:
                        ...
                        if (
                            ...
                            ks_timezone = self._context.get("tz") or self.env.user.tz
                            selected_start_date = ks_convert_into_local(
                                rec.ks_item_start_date_2, ks_timezone
                            )
                            selected_end_date = ks_convert_into_local(
                                rec.ks_item_end_date_2, ks_timezone
                            )
    ...
    def ks_fetch_chart_data(
        ...
        if ks_chart_groupby_type == "date_type":
            ks_chart_groupby_field = (
                ks_chart_groupby_relation_field + ":" + ks_chart_date_groupby
            )
        else:
            ks_chart_groupby_field = ks_chart_groupby_relation_field

        try:
            if self.ks_fill_temporal and ks_chart_date_groupby not in [
                "minute",
                "hour",
            ]:
                ks_chart_records = (
            ...
            if all(measure_field in res for measure_field in ks_chart_measure_field):
                if ks_chart_groupby_type == "relational_type":
                    if res[ks_chart_groupby_field]:
                        ks_chart_data["groupByIds"].append(
                            res[ks_chart_groupby_field][0]
                        )
                    ...
                    if selection:
                        label = dict(
                            self.env[ks_model_name].fields_get(
                                allfields=[ks_chart_groupby_field]
                            )[ks_chart_groupby_field]["selection"]
                        )[selection]
                    else:
                        label = selection
                else:
                ...
                if ks_chart_measure_field:
                    if ks_chart_measure_field_2:
                        ...
                        for field_rec in ks_chart_measure_field_2:
                            ks_groupby_equal_measures = (
                                res.get(
                                    ks_chart_groupby_relation_field + "_count", False
                                )
                                ...
                                or res.get("__count", False)
                                and ks_chart_measure_field_2_ids[index]
                                == ks_chart_groupby_relation_field_id
                                else 1
                            )
                            try:
                                ...
                                if is_ks_index:
                                    if chart_count == "sum":
                                        ks_chart_data["datasets"][counter]["data"][
                                            ks_index
                                        ] += data
                                    else:
                                        ks_chart_data["datasets"][counter]["data"][
                    ...
                    for field_rec in ks_chart_measure_field:
                        ks_groupby_equal_measures = (
                            res.get(ks_chart_groupby_relation_field + "_count", False)
                            or res.get("__count", False)
                            if res.get(
                                ks_chart_groupby_relation_field + "_count", False
                            )
                            or res.get("__count", False)
                            and ks_chart_measure_field_ids[index]
                            == ks_chart_groupby_relation_field_id
                            else 1
                        )
                        try:
    ...
    def ks_fetch_drill_down_data(self, item_id, domain, sequence):
        ...
        ks_chart_type = (
            action_line.ks_chart_type
            if action_line.ks_chart_type
            else record.ks_dashboard_item_type
        )
        ks_list_view_data = {
            "label": [],
            "type": "grouped",
        ...
        if action_line.ks_chart_type == "ks_list_view":
            ...
            orderby = (
                action_line.ks_sort_by_field.name
                if action_line.ks_sort_by_field
                else "id"
            )
            ...
            if action_line.ks_item_action_field.ttype == "many2one":
                ...
                for res in ks_list_view_records:
                    ...
                    for field_rec in ks_list_fields:
                        if counter == 0:
                            data_row["data"].append(
                                res[field_rec][1]._value if res[field_rec] else "False"
                            )
                        else:
                            data_row["data"].append(res[field_rec])
                        counter += 1
                ...
                action_line.ks_item_action_field.ttype == "date"
                or action_line.ks_item_action_field.ttype == "datetime"
            ):
                ks_list_view_data["list_view_type"] = "date_type"
                ks_list_field = []
                ks_list_view_data["groupby"] = (
                ...
                for res in ks_list_view_records:
                    ...
                    if res[ks_selection_field]:
                        data_row["data"].append(
                            dict(
                                self.env[record.ks_model_name].fields_get(
                                    allfields=ks_selection_field
                                )[ks_selection_field]["selection"]
                            )[res[ks_selection_field]]
                        )
                ...
                for ks_multiplier in record.ks_multiplier_lines:
                    ...
                    if label in ks_list_view_data["label"]:
                        ...
                        for i in range(0, len(ks_list_view_data["data_rows"])):
                            ...
                            ks_list_view_data["data_rows"][i]["data"][
                                index
                            ] = data_values
            return {
                "ks_list_view_data": json.dumps(ks_list_view_data),
            ...
            if record.ks_chart_data_count_type == "count":
                if not action_line.ks_sort_by_field:
                    ...
                    if not action_line.ks_sort_by_field.ttype == "datetime":
                        ks_chart_measure_field_with_type.append(
                            action_line.ks_sort_by_field.name + ":" + "sum"
                        )
                    else:
                        ks_chart_measure_field_with_type.append(
                            action_line.ks_sort_by_field.name
    ...
    def ks_get_start_end_date(
        ...
        if model_field_start_date and goal_model_start_date:
            ...
            if model_field_start_date < goal_model_start_date:
                ks_start_end_date["start_date"] = model_field_start_date.strftime(
                    "%Y-%m-%d 00:00:00"
                )
            else:
                ks_start_end_date["start_date"] = goal_model_start_date.strftime(
                    "%Y-%m-%d 00:00:00"
                )
            if model_field_end_date > goal_model_end_date:
                ks_start_end_date["end_date"] = model_field_end_date.strftime(
                    "%Y-%m-%d 23:59:59"
                )
            else:
                ks_start_end_date["end_date"] = goal_model_end_date.strftime(
                    "%Y-%m-%d 23:59:59"
                )
    ...
    def ks_get_next_offset(self, ks_item_id, offset, item_domain=[]):
        ...
        ks_list_domain = self.ks_convert_into_proper_domain(
            record.ks_domain, self, item_domain
        )
        if self.ks_list_view_type == "grouped":
            orderby = record.ks_sort_by_field.id
            sort_order = record.ks_sort_by_order
            ks_list_view_data = self.get_list_view_record(
                orderby, sort_order, ks_list_domain, ksoffset=int(ks_offset)
            )

        else:
            ks_list_view_data = self.ks_fetch_list_view_data(
                record, ks_list_domain, offset=int(ks_offset)
            )
            ...
            for data in ks_data:
                if field_type == "date_type":
                    if ks_chart_date_groupby in ["minute", "hour"]:
                        if ks_chart_date_sub_groupby in [
                            "month",
                            "week",
                            "quarter",
                            "year",
                            ...
                            data["value"].sort(
                                key=lambda x: int(
                                    str(
                                        ks_sorted_months.index(x["x"].split(" ")[2]) + 1
                                    )
                                    + x["x"].split(" ")[1]
                                    + x["x"].split(" ")[0].replace(":", "")
                                ),
                        ...
                        ks_chart_date_groupby == "day"
                        and ks_chart_date_sub_groupby in ["quarter", "year"]
                    ):
                        ...
                        ks_chart_date_groupby == "day"
                        and ks_chart_date_sub_groupby not in ["quarter", "year"]
                    ):
        ...
        if self.ks_domain_2:
            ...
            try:
                ...
                if "%MYCOMPANY" in ks_domain_2:
                    ks_domain_2 = ks_domain_2.replace(
                        "%MYCOMPANY", str(self.env.company.id)
                    )
...
class KsDashboardItemsGoal(models.Model):
    ...
    ks_dashboard_item = fields.Many2one(
        "ks_dashboard_ninja.item", string="Dashboard Item"
    )
...
class KsDashboardItemsActions(models.Model):
    ...
    ks_item_action_field_type = fields.Char(
        compute="ks_get_item_action_type", compute_sudo=False
    )
    ...
    ks_dashboard_item_id = fields.Many2one(
        "ks_dashboard_ninja.item", string="Dashboard Item"
    )
    ks_model_id = fields.Many2one(
        "ir.model", related="ks_dashboard_item_id.ks_model_id"
    )
    ...
    def ks_get_item_action_type(self):
        for rec in self:
            if (
                rec.ks_item_action_field.ttype == "datetime"
                or rec.ks_item_action_field.ttype == "date"
            ):
    ...
    def ks_check_date_group_by(self):
        for rec in self:
            if (
                rec.ks_item_action_field.ttype == "date"
                and rec.ks_item_action_date_groupby in ["hour", "minute"]
            ):
                raise ValidationError(
                    _("Action field: {} cannot be aggregated by {}").format(
                        rec.ks_item_action_field.display_name,
                        rec.ks_item_action_date_groupby,
                    )
                )
