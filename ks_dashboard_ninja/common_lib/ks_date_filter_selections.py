...
def ks_date_series_t(ks_date_selection, timezone, type, self=None):
    return eval("ks_get_date_range_from_" + ks_date_selection)(
        "current", timezone, type, self
    )
...
def ks_get_date_range_from_future(date_state, self_tz, type, self):
    ...
    if type == "date":
        ks_date_data["selected_start_date"] = date.strptime(
            date.strftime("%Y-%m-%d"), "%Y-%m-%d"
        )
    else:
...
def ks_get_date_range_from_futurestarting(date_state, self_tz, type, self):
    ...
    start_date = datetime.strptime(
        date.strftime("%Y-%m-%d 00:00:00"), "%Y-%m-%d %H:%M:%S"
    )
    if type == "date":
        ks_date_data["selected_start_date"] = datetime.strptime(
            start_date.strftime("%Y-%m-%d"), "%Y-%m-%d"
...
def ks_convert_into_utc(datetime, timezone):
    ...
    return (
        ks_tz.localize(datetime.replace(tzinfo=None), is_dst=False)
        .astimezone(pytz.UTC)
        .replace(tzinfo=None)
    )
