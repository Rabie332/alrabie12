odoo.define("account.ReconciliationRenderer", function (require) {
    ...
    var StatementRenderer = Widget.extend(FieldManagerMixin, {
        ...
        _renderNotifications: function (notifications) {
            ...
            for (var i = 0; i < notifications.length; i++) {
                ...
                $notification
                    .appendTo(this.$(".notification_area"))
                    .slideDown(300);
            }
        },
        ...
        _onGoToBankStatement: function (e) {
            ...
            this.do_action({
                ...
                type: "ir.actions.act_window",
                context: {
                    search_default_journal_id: journalId,
                    journal_type: "bank",
                },
                view_mode: "form",
            });
    ...
    var LineRenderer = Widget.extend(FieldManagerMixin, {
                ...
                function (recordID) {
                    self.fields.ref = new basic_fields.FieldChar(
                        ...
                        self.model.get(recordID),
                        {
                            ...
                            attrs: {
                                placeholder:
                                    self._initialState.st_line.ref ||
                                    _t("Reference"),
                            },
                        }
                    );
        ...
        update: function (state) {
            ...
            for (let i = 0; i < matching_modes.length; i++) {
                ...
                this.$(
                    '.o_notebook li a[href*="notebook_page_' +
                        matching_modes[i] +
                        '"]'
                )
                    .parent()
                    ...
                    if (!isNaN(line.id)) {
                        $('<span class="line_info_button fa fa-info-circle"/>')
                            ...
                            .attr(
                                "data-content",
                                qweb.render(
                                    "reconciliation.line.mv_line.details",
                                    {
                                        line: line,
                                    }
                                )
                ...
                createPromise.then(function () {
                    ...
                    return self.model
                        ...
                        .then(function () {
                            ...
                            return self.model.notifyChanges(
                                self.handleCreateRecord,
                                {
                                    analytic_tag_ids: {
                        ...
                        .then(function () {
                            var defs = [];
                            _.each(state.createForm.analytic_tag_ids, function (
                                tag
                            ) {
                                defs.push(
            ...
            return this.model
                ...
                .then(function (recordID) {
                    ...
                    self.fields.account_id
                        .appendTo(
                            $create.find(".create_account_id .o_td_field")
                        )
                        .then(
                            addRequiredStyle.bind(self, self.fields.account_id)
                        );
                    ...
                    self.fields.date.appendTo(
                        $create.find(".create_date .o_td_field")
                    );
        ...
        _onFieldChanged: function (event) {
            ...
            if (fieldName === "partner_id") {
                ...
                if (
                    event.data.changes.amount &&
                    isNaN(event.data.changes.amount)
                ) {
                    return;
                }
                this.trigger_up("update_proposition", {
        ...
        _onInputKeyup: function (event) {
            var target_partner_id = $(event.target).parents(
                '[name="partner_id"]'
            );
            ...
            if (event.keyCode === 13) {
                ...
                var created_lines = _.findWhere(this.model.lines, {
                    mode: "create",
                });
    ...
    var ManualLineRenderer = LineRenderer.extend({
        ...
        update: function (state) {
            this._super(state);
            var props = _.filter(state.reconciliation_proposition, {
                display: true,
            });
            if (!props.length) {
                var $line = $(
