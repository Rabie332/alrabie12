    ...
    var StatementAction = AbstractAction.extend({
            ...
            switch (ev.data.direction) {
                case "up":
                    ...
                    widget = this._getWidget(
                        non_reconciled_keys[currentIndex - 1]
                    );
                    break;
                case "down":
                    ev.stopPropagation();
        ...
        willStart: function () {
            ...
            return def.then(function () {
                ...
                return promise.then(function (result) {
                    ...
                    self.renderer = new self.config.ActionRenderer(
                        self,
                        self.model,
                        {
                            bank_statement_line_id:
                                self.model.bank_statement_line_id,
        ...
        do_show: function () {
            ...
            if (this.action_manager) {
                this.$pager = $(
                    QWeb.render("reconciliation.control.pager", {
                        widget: this.renderer,
                    })
                );
            ...
            var handle = _.compact(
                _.map(this.model.lines, function (line, handle) {
                    return line.reconciled ||
                        parseInt(handle.substr(5)) <
                            parseInt(previous_handle.substr(5))
                        ? null
                        : handle;
                })
            )[0];
        ...
        _renderLines: function () {
            ...
            _.each(linesToDisplay, function (line, handle) {
                var widget = new self.config.LineRenderer(
                    self,
                    self.model,
                    line
                );
        ...
        _onActionPartialAmount: function (event) {
            var self = this;
            var handle = event.target.handle;
            var amount = this.model.getPartialReconcileAmount(
                handle,
                event.data
            );
            self._getWidget(handle).updatePartialAmount(
                event.data.data,
                amount
            );
        },
        ...
        _onValidate: function (event) {
            ...
            this.model.validate(handle).then(function (result) {
                ...
                _.each(result.handles, function (handle) {
                    ...
                    if (widget) {
                        widget.destroy();
                        var index = _.findIndex(self.widgets, function (
                            widget
                        ) {
                            return widget.handle === handle;
                        });
                ...
                if (
                    ...
                    var toLoad =
                        self.model.defaultDisplayQty - self.widgets.length;
    ...
    var ManualAction = StatementAction.extend({
        ...
        _onValidate: function (event) {
            ...
            this.model[method](handle).then(function (result) {
                ...
                self.renderer.update({
                    valuenow: _.compact(_.invoke(self.widgets, "isDestroyed"))
                        .length,
