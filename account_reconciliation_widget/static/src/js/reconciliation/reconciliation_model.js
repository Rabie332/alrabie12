odoo.define("account.ReconciliationModel", function (require) {
    ...
    var StatementModel = BasicModel.extend({
        ...
        init: function (parent, options) {
            ...
            this.domain = [];
            this.defaultDisplayQty =
                (options && options.defaultDisplayQty) || 10;
        ...
        addProposition: function (handle, mv_line_id) {
            ...
            var prop = _.clone(
                _.find(line["mv_lines_" + line.mode], {id: mv_line_id})
            );
        ...
        changePartner: function (handle, partner, preserveMode) {
            ...
            return Promise.resolve(
                partner && this._changePartner(handle, partner.id)
            )
                .then(function () {
                    if (line.st_line.partner_id) {
                        _.each(line.reconciliation_proposition, function (
                            prop
                        ) {
        ...
        getStatementLines: function () {
            var self = this;
            var linesToDisplay = _.pick(this.lines, function (
                value,
                key,
                object
            ) {
            ...
            this._rpc({
                ...
                this._rpc({
                    ...
                    for (const reconcileModelLine of reconcileModelLines) {
                        const reconcileModelByLine = _.findWhere(
                            reconcileModels,
                            {
                    ...
                    this._readAnalyticTags({res_ids: analyticTagIds}).then(
                        () => {
                            for (const reconcileModel of reconcileModels) {
                                var analyticTagData = [];
                                var modelTags =
                                    reconcileModel.analytic_tag_ids || [];
        ...
        partialReconcile: function (handle, data) {
            var line = this.getLine(handle);
            var prop = _.find(line.reconciliation_proposition, {
                id: data.mvLineId,
            });
            if (prop) {
                ...
                if (fieldName === "analytic_tag_ids") {
                    switch (value.operation) {
                        case "ADD_M2M":
                            ...
                            var vids = _.isArray(value.ids)
                                ? value.ids
                                : [value.ids];
                            _.each(vids, function (val) {
                                if (
                                    !_.findWhere(prop.analytic_tag_ids, {
                                        id: val.id,
                                    })
                                ) {
                    ...
                    switch (value.operation) {
                        ...
                        case "FORGET":
                            prop.__tax_to_recompute = true;
                            var id = self.localData[value.ids[0]].ref;
                            prop.tax_ids = _.filter(prop.tax_ids, function (
                                val
                            ) {
                                return val.id !== id;
                            });
        ...
        validate: function (handle) {
            ...
            _.each(handles, function (handle) {
                ...
                var props = _.filter(line.reconciliation_proposition, function (
                    prop
                ) {
                    return !prop.invalid;
                });
                ...
                handlesPromises.push(
                    Promise.resolve(computeLinePromise).then(function () {
                        var values_dict = {
                            ...
                            counterpart_aml_dicts: _.map(
                                _.filter(props, function (prop) {
                                    return (
                                        !isNaN(prop.id) &&
                                        !prop.is_liquidity_line
                                    );
                                }),
                                self._formatToProcessReconciliation.bind(
                                    self,
                                    line
                                )
                            ),
                        ...
                        var currency = session.get_currency(
                        ...
                        if (!utils.float_is_zero(balance, currency.digits[1])) {
                            ...
                            values_dict.new_aml_dicts.push(
                                unreconciled_amount_dict
                            );
                        }
        ...
        _computeLine: function (line) {
            ...
            _.each(line.reconciliation_proposition, function (prop) {
                ...
                if (prop.tax_repartition_line_id) {
                    if (
                        !_.find(line.reconciliation_proposition, {
                            id: prop.link,
                        }).__tax_to_recompute
                    ) {
                ...
                if (
                    ...
                    reconciliation_proposition = _.filter(
                        ...
                        function (p) {
                            return (
                                !p.tax_repartition_line_id || p.link !== prop.id
                            );
                        }
                    );
                    var args = [
                        ...
                        self
                            ...
                            .then(function (result) {
                                _.each(result.taxes, function (tax) {
                                    var tax_prop = self._formatQuickCreate(
                                        line,
                                        {
                                            link: prop.id,
                                            tax_ids: tax.tax_ids,
            ...
            return Promise.all(tax_defs).then(function () {
                ...
                if (isOtherCurrencyId) {
                    var other_currency = session.get_currency(
                        isOtherCurrencyId
                    );
                    var other_precision =
                ...
                line.balance = {
                    ...
                    amount_currency: isOtherCurrencyId
                        ? amount_currency
                        : total,
        ...
        _formatMoveLine: function (handle, mode, mv_lines) {
            ...
            if (mv_lines[0]) {
                line["remaining_" + mode] =
                    mv_lines[0].recs_count - mv_lines.length;
        ...
        _formatQuickCreate: function (line, values, reconcileModel) {
            ...
            var analytic_account = this._formatNameGet(
            ...
            var prop = {
                ...
                tax_tag_ids: this._formatMany2ManyTagsTax(
                    values.tax_tag_ids || []
                ),
                ...
                base_amount: amount,
                percent:
                    values.amount_type === "percentage" ? values.amount : null,
                link: values.link,
                display: true,
                invalid: true,
            ...
            if (prop.base_amount) {
                ...
                prop.base_amount =
                    sign *
                    field_utils.parse.monetary(amount, {}, formatOptions);
            }
        ...
        _formatToProcessReconciliation: function (line, prop) {
            ...
            var result = {
                ...
                analytic_tag_ids: [
                    [6, null, _.pluck(prop.analytic_tag_ids, "id")],
                ],
            };
            if (!isNaN(prop.id)) {
            ...
            if (prop.tax_tag_ids && prop.tax_tag_ids.length)
                result.tax_tag_ids = [
                    [6, null, _.pluck(prop.tax_tag_ids, "id")],
                ];
            if (prop.tax_repartition_line_id)
    ...
    var ManualModel = StatementModel.extend({
        ...
        loadMore: function (qty) {
            ...
            var lines = this.manualLines.slice(
                this.pagerIndex,
                this.pagerIndex + qty
            );
            this.pagerIndex += qty;
        ...
        _formatLine: function (type, data) {
            var line = (this.lines[_.uniqueId("rline")] = _.extend(data, {
                ...
                account_id: this._formatNameGet([
                    data.account_id,
                    data.account_name,
                ]),
                st_line: data,
        ...
        _formatMoveLine: function (handle, mode, mv_lines) {
            ...
            if (
                ...
                line.mode =
                    this.avoidCreate || !line.balance.amount
                        ? "inactive"
                        : "create";
                if (line.mode === "create") {
                    return this._computeLine(line).then(function () {
