...
class HrPayslip(models.Model):
    ...
    is_specific_struct = fields.Boolean(
        string="Specific payslip",
        help="you can generate an extra payslip for specific bonus (eg: End of the year bonus) "
        "which generate a payslip with a specific rules",
    )
    ...
    def action_payslip_done(self):
        # we don't need to calculate sheet when confirm it,
        # to give for user away to edit amount deduction of some lines
        # self.compute_sheet()
    ...
    @api.model
    def _get_payslip_lines(self, contract_ids, payslip_id):  # noqa: C901
        ...
        baselocaldict = {
            **self._get_base_local_dict(payslip),
            **{
    ...
    @api.onchange("employee_id", "date_from", "date_to", "is_specific_struct")
    def onchange_employee(self):
        ...
        if not self.env.context.get("contract") or not self.contract_id:
            ...
            if not contract_ids:
                return
            # todo: Check the commented line :
            # "Use the selected contract NOT contract number 0!"
        ...
        # todo: Check this: "we are only going to fetch the selected contract!".
        if self.contract_id:
...
class HrPayslipRun(models.Model):
    ...
    is_specific_struct = fields.Boolean(
        string="Specific Lot",
        help="you can generate an extra lots of payslips for specific bonus "
        "(eg: End of the year bonus)"
        " which generate a payslip with a specific rules",
    )
    ...
    def get_employees_domain(self):
        ...
        if self.category_ids:
            domain += [("category_ids", "in", self.category_ids.ids)]
        return domain

    def generate_employees_sheet(self):
        ...
        for employee in employees:
            ...
            res = {
                ...
                "company_id": employee.company_id.id,
                "is_specific_struct": self.is_specific_struct,
            }
