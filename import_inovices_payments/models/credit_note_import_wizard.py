...
class CreditNoteImportWizard(models.TransientModel):
    _name = 'credit.note.import.wizard'
    _description = 'Credit Note Import Wizard'
    ...
    company = fields.Many2one(
        'res.company', required=True, string='Credit note Company')

    journal = fields.Many2one(
        'account.journal', required=True, string='Credit note Journal')

    taxes = fields.Many2many(
        'account.tax', string='Credit note Taxes')
    ...
    def process_file(self, encoded_file_data):
        ...
        for row in range(1, sheet.nrows):
            try:
                ...
                credit_note_date = xlrd.xldate.xldate_as_datetime(
                ...
                _logger.info(
                    f"Parsed values - Partner: {partner_name}, Date: {credit_note_date}, Label: {label}, Net Value: {net_value}")
                ...
                credit_note_vals = {
                    ...
                    'invoice_date': credit_note_date.isoformat(),
                    'journal_id': self.journal.id,
                    'move_type': 'out_refund',
                    'invoice_line_ids': [(0, 0, credit_note_line_vals)],
                    'company_id': self.company.id,
                }
                self.create_single_credit_note(credit_note_vals)
    ...
    @api.model
    def create_single_credit_note(self, credit_note_vals):
        try:
            credit_note = self.env['account.move'].sudo().create(
                credit_note_vals)
            credit_note.sudo().action_post()
            _logger.info(f'Successfully created credit_note {credit_note.id}')
        except Exception as e:
            _logger.error(f"Failed to create and post credit_note: {e}")
            raise
